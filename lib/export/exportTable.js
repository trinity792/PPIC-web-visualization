 ...
 *                                 showing, so the CSV, the .xlsx, and what the
 *                                 reader sees are one table
...
/**
 * The filename the CSV and XLSX exports agree on: `<module>-<chartType>.csv`.
 * The retired R/Stata generators read it too, so their `read_csv(...)` header
 * named the file a reader would actually download - kept as the shape rather
 * than reinvented, in case that export item is ever built.
 */
function tableFilename(spec) {
  const module = spec?.question?.dataset?.moduleId || spec?.module || "data";
  const chartType = spec?.presentation?.chartType || spec?.chartType || "table";
  return `${module}-${chartType}.csv`;
}
...
/** Build the shared Data view / CSV / XLSX table from v3 observations. */
export function displayTableFromObservations({
  observations = [],
  presentation = {},
  scope = "all",
} = {}) {
  const selected =
    scope === "visibleTab" && presentation.activeTab
      ? observations.filter((row) => row.comparisonId === presentation.activeTab)
      : observations;
  const valueName = selected[0]?.measureLabel || selected[0]?.measureId || "Value";
  const hasCategory = selected.some((row) => row.categoryLabel != null);
  const columns = [
    column("Comparison", "text"),
    column("Geography", "text"),
    ...(hasCategory ? [column("Category", "text")] : []),
    column("Period", "text"),
    column(valueName, "number"),
    column("Status", "text"),
    column("Value kind", "text"),
    column("Calculation", "text"),
    column("Included years", "text"),
    column("Source", "text"),
  ];
  const values = (row, forExport = false) => [
    row.comparisonLabel,
    row.geographyLabel,
    ...(hasCategory ? [row.categoryLabel] : []),
    row.period,
    ...
    row.valueKind,
    row.calculation?.id,
    (row.includedPeriods || []).join("; "),
    row.source,
  ];
  return {
    filename: "visualization-data.csv",
    columns,
    rows: selected.map((row) => values(row)),
    exportRows: selected.map((row) => values(row, true)),
  };
}
...
export function displayTable(spec, loaded = {}) {
  if (spec?.version === 3) {
    return displayTableFromObservations({
  ...
  // Range/forest: one row per unit with its two endpoints (+ optional center
  // point/estimate and, for forest plots, a study weight).
  if (spec?.chartType === "dumbbell" || spec?.chartType === "forest") {
...
export function originalTable(spec, loaded = {}) {
  const inline = spec?.question?.dataset?.inline || spec?.data?.inline;
  ...
  // `?? {}` rather than a default parameter: callers pass an explicit `null`
  // result whenever no chart has been loaded (an unarmed module workbench), and
  // a default only fills in for `undefined`.
...
export function toCsv(table) {
  ...
  const rows = table?.exportRows || table?.rows || [];
