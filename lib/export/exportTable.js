...
export function displayTable(spec, loaded = {}) {
  ...
  if (
    ...
    (matrix.y || []).forEach((rowName, r) => {
      (matrix.x || []).forEach((colName, c) => {
        rows.push([rowName, colName, matrix.z?.[r]?.[c] ?? null]);
      });
    });
    return { filename, columns, rows };
  }
  ...
  // Range/slope: one row per unit with its two endpoints (+ optional center point).
  if (spec?.chartType === "dumbbell" || spec?.chartType === "slope") {
...
/** One-sheet .xlsx workbook (ExcelJS lazy-imported so it stays out of the bundle). */
export async function toXlsxBlob(table) {
