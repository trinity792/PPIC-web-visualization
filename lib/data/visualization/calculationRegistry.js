...
/** Change for every adjacent pair, plotted at the later period. */
function adjacentChangeCalculation(id, { observations, params = {}, comparisonId }) {
  ...
  const label =
    id === "percentChange"
      ...
      : "Percentage-point change";
  const unit = id === "percentChange" ? "percent" : "percentagePoints";
  if (ordered.length < 2) {
    return {
      rows: [],
      issues: [
        comparisonIssue(
          "twoPeriodsRequired",
          comparisonId,
          `${label} needs at least two ordered periods.`,
        ),
      ],
    };
  }
  ...
  for (let index = 1; index < ordered.length; index += 1) {
    ...
    if (id === "percentChange" && previous.value === 0) {
      rows.push(
        derivedRow(current, {
          id,
          params,
    ...
    rows.push(
      derivedRow(current, {
        id,
        params,
        value: calculateChangeValue(previous.value, current.value, id),
        includedPeriods,
        unit,
      }),
    );
...
export function applyCalculation(id, context) {
  ...
  if (
    ...
    (id === "percentagePointChange" &&
      ["range", "orderedSequence"].includes(context.timeContract))
  ) {
    return adjacentChangeCalculation(id, next);
  }
