    ...
    for (const category of categories) {
      for (const period of periods) {
        ...
        rows.push({
          ...
          // Every observation requires a source (observationContract.js), even
          // a materialized missing/suppressed cell with no matching row. Most
          // modules have an explicit `question.source`; the few that don't
          // (no `sources` axis) fall back to the adapter's own label.
          source: found?.source || comparison.source || question.source || adapter.label || adapter.id,
        });
      }
    }
  }
...
const TWO_PERIOD_CALCULATIONS = new Set(["numericChange", "percentagePointChange"]);
const SEQUENCE_CONTRACTS = new Set(["range", "orderedSequence"]);

export function calculationParamsFor(
  calculation = {},
  activePeriods = [],
  timeContract,
) {
  const params = { ...(calculation.params || {}) };
  const adjacentPercentagePoints =
    calculation.id === "percentagePointChange" &&
    SEQUENCE_CONTRACTS.has(timeContract);
  if (TWO_PERIOD_CALCULATIONS.has(calculation.id) && !adjacentPercentagePoints) {
...
export async function executeQuestion(spec, { adapter } = {}) {
  ...
  for (const comparison of requestedComparisons) {
    ...
    const activeTime = comparison.time || question.time;
    const comparisonTime = comparison.time
      ? periodsFor(activeTime, availablePeriods, await adapter.defaultReportingPeriod(question))
      : { periods };
    ...
    const calculationParams = calculationParamsFor(
      calculation,
      activePeriods,
      activeTime?.contract,
    );
    for (const group of byGeography(cells)) {
      const result = applyCalculation(calculation.id || "actual", {
        ...
        comparisonId: comparison.id,
        timeContract: activeTime?.contract,
      });
    ...
    returned.forEach((row) => returnedPeriods.add(row.period));
