...
import { queryVisualizationTable as queryBuildingPermitsTable } from "@/lib/data/building_permits";
...
/**
 * A comparison must pin every dimension its schema declares. Otherwise the
 * row filter in `select()` lets every stratum through and `materialize()`
 * silently keeps whichever row the CSV lists first — a wrong number, not an
 * error. Returns the missing dimensions' labels, empty when complete.
 */
function missingComparisonDimensions(schema, comparison) {
  return (schema.comparisonDimensions || [])
    .filter((dimension) => {
      const value = comparison.dimensions?.[dimension.id];
      return value == null || value === "" || (Array.isArray(value) && !value.length);
    })
    .map((dimension) => dimension.label || schema.fields?.[dimension.id]?.label || dimension.id);
}

function incompleteComparisonIssue(comparison, missing) {
...
export const projectionsAdapter = Object.freeze({
  id: "projections",
  // Fallback observation source when a materialized cell has no matching row
  // (a genuinely missing/suppressed period-geography combination) and no
  // explicit `source` is set on the comparison or question — see
  // `materialize()` in executeQuestion.js. Every module always has one of
  // these two, but for Projections `question.source` is always set (a
  // mandatory DoF P-3 / Census cc-est choice), so this never actually fires.
  label: DEMOGRAPHIC_PROJECTIONS_SCHEMA.label,
  ...
  validateComparison({ question, comparison }) {
    ...
    const missing = missingComparisonDimensions(DEMOGRAPHIC_PROJECTIONS_SCHEMA, comparison);
    if (missing.length) return [incompleteComparisonIssue(comparison, missing)];
...
export const componentsOfChangeAdapter = Object.freeze({
  ...
  // See the matching comment on `projectionsAdapter.label` above.
  label: COMPONENTS_OF_CHANGE_SCHEMA.label,
  ...
  validateComparison({ question, comparison }) {
    const geography = comparison.geography || question.geography;
    const source = comparison.source || question.source;
    const requiredSource = COMPONENTS_OF_CHANGE_SCHEMA.subsetSource[geography.subset];
    if (requiredSource && source !== requiredSource) {
      return [
        comparisonIssue(
          "invalidSourceForSubset",
          comparison.id,
          `${geography.subset} is only available from ${requiredSource}.`,
        ),
      ];
    }
    return [];
...
function genericModuleAdapter(schema, loadTable) {
  ...
  return Object.freeze({
    id: schema.id,
    // See the matching comment on `projectionsAdapter.label` above. Modules
    // wired through this generic adapter (PopHousing, Housing Stress,
    // Building Permits, RHNA Progress) mostly have no `sources` axis, so
    // `question.source` is routinely undefined — this is the fallback that
    ...
    comparisonLabel(question, comparison) {
      ...
      const omitValues = schema.labelMeta?.omitValues || {};
      // Drop aggregate/base values (e.g. "All" race, "Total" tenure) the same
      // way the demographic-projections and housing-stress-style comparison
      // cards do, so a default comparison reads "Alameda" rather than
      // "Alameda All Total".
      const values = Object.entries(comparison.dimensions || {}).flatMap(([key, value]) =>
        (Array.isArray(value) ? value : [value]).filter(
          (entry) => entry && !omitValues[key]?.includes(entry),
        ),
      );
    ...
    validateComparison({ comparison }) {
      // Housing Stress and RHNA Progress declare comparison dimensions; the
      // location-only modules declare none and pass straight through.
      const missing = missingComparisonDimensions(schema, comparison);
      return missing.length ? [incompleteComparisonIssue(comparison, missing)] : [];
    },
    async select({ question, comparison, measure, periods }) {
      const geography = comparison.geography || question.geography;
      const locations = geography?.locations || [];
