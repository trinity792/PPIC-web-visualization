...
function parseCsv(text) {
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    row[DERIVED_MEASURE] = MULTIFAMILY_PARTS.every(
      (part) => row[part] != null,
    )
      ? MULTIFAMILY_PARTS.reduce((sum, part) => sum + row[part], 0)
      : null;
    rows.push(row);
  }
  return rows;
...
function addMeasure(accumulator, row, measure) {
  accumulator[measure] =
    accumulator[measure] == null || row[measure] == null
      ? null
      : accumulator[measure] + row[measure];
}
...
function buildRestOfUs(stateRows) {
  ...
  for (const row of stateRows) {
    ...
    for (const measure of RAW_MEASURES) addMeasure(accumulator, row, measure);
  }
...
function buildTwoPeriod(rows, measure, { startMonth, endMonth }) {
  ...
  const records = [...byLocation.entries()]
    ...
    .sort((a, b) => {
      if (a.numericChange == null && b.numericChange == null) {
        return a.location.localeCompare(b.location);
      }
      if (a.numericChange == null) return 1;
      if (b.numericChange == null) return -1;
 ...
 * geographic multi-select. Resolved through `subsetRows` so the derived
 * locations this module invents are included: the "Rest of US" state row and
 * the nine metro-aggregated regions.
 */
...
/**
 * Every row the v3 observation adapter may select from: the stored State and
 * Metro rows plus the derived "Rest of US" state and the 9 region aggregates
 * (`Geographic Level` "Region"). The `?view=locations` list offers those
 * derived places, so the adapter must be able to answer for them too — the
 * raw `full` table above deliberately cannot.
 */
export async function queryVisualizationTable() {
  const rows = await loadBuildingPermitsData();
  const records = AVAILABLE_SUBSETS.flatMap((subset) => subsetRows(rows, subset));
  return { records, rowCount: records.length };
}
