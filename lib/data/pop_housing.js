...
// Provenance labels the output can carry (E-5 / E-8 / Aggregated). The UI offers
// these as a multi-select filter that defaults to all (refactor guide B3).
export const AVAILABLE_SOURCES = POPHOUSING_SCHEMA.fields.Source.values;
...
/**
 * Distinct location names for one geographic subset, for the sidebar's
 * geographic multi-select. Provenance (E-5 / E-8 / Aggregated) is deliberately
 * ignored: the list describes the geography, not the vintage.
 */
export async function queryLocations({ subset }) {
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  const rows = filterRows(await loadPopHousingData(), { levels });
  return { subset, locations: buildLocationList(rows) };
}
...
export async function queryCategoryValues({
  ...
  const rows = await filteredRows({ subset, locations, sources });
...
export async function queryMatrix({
  ...
  validateMeasure(parameter);
  const rows = await filteredRows({ subset, locations, sources, startYear, endYear });
...
export async function queryGeoValues({
  ...
  const rows = await filteredRows({ subset, sources });
...
// Department of Finance products backing the dashboard's stats and region table
// (the derived "Aggregated" blend is intentionally omitted from the footnote).
const DOF_SOURCE_LABELS = {
  "E-5": "California Dept. of Finance E-5 (population & housing estimates)",
  "E-8": "California Dept. of Finance E-8 (historical benchmark)",
};

/**
 * Per-source data vintage: the latest Year present for each real DoF source
 * (E-5, E-8). Written for the retired landing dashboard's footnote; it has no
 * caller in the app since that surface moved to `.trash/landing-overhaul/`, and
 * is kept as a query helper with its own coverage.
 */
export async function queryDataSources() {
  const rows = await loadPopHousingData();
  const maxYear = new Map();
  for (const row of rows) {
    if (!DOF_SOURCE_LABELS[row.Source]) continue;
    const current = maxYear.get(row.Source);
    if (current === undefined || row.Year > current) maxYear.set(row.Source, row.Year);
  }
  return ["E-5", "E-8"]
    .filter((source) => maxYear.has(source))
    .map((source) => ({
      label: DOF_SOURCE_LABELS[source],
      lastUpdated: `data through ${maxYear.get(source)}`,
    }));
}
...
export async function queryFullTable({
  ...
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  return buildFullTable(rows, { levels, source: sources, locations, startYear, endYear });
}
