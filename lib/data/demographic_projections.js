...
/**
 * Read only the rows a v3 comparison can use. The full projections file has
 * more than 1.7 million rows; parsing every row into an object for a request
 * that names one or two places creates avoidable latency and memory pressure.
 */
export async function selectProjectionRows({
  levels,
  source,
  locations = [],
  periods = [],
  ...
  cachedText ||= await readFile(DATA_PATH, "utf8");
  cachedLines ||= cachedText.split(/\r?\n/);
  const lines = cachedLines;
  const selected = [];
  const periodSet = new Set(periods);
  const levelSet = new Set(levels);
  const locationSet = new Set(locations);
  const locationNeedles = locations.map((location) => `,${location},`);
  for (let index = 1; index < lines.length; index += 1) {
    const line = lines[index];
    if (!line || !line.endsWith(`,${source}`)) continue;
    if (locationSet.size && !locationNeedles.some((needle) => line.includes(needle))) {
      continue;
    }
    const cells = parseCsvLine(line);
    const year = Number.parseInt(cells[2], 10);
    if (!levelSet.has(cells[0]) || !periodSet.has(year) || (locationSet.size && !locationSet.has(cells[1]))) {
      continue;
    }
    const rowDimensions = {
      "Age Group": cells[3],
      Sex: cells[4],
      "Race/Ethnicity": cells[5],
    };
    if (
      !Object.entries(dimensions).every(([key, value]) =>
        Array.isArray(value) ? value.includes(rowDimensions[key]) : rowDimensions[key] === value,
      )
    ) {
      continue;
    }
    selected.push({
      "Geographic Level": cells[0],
      Location: cells[1],
      Year: year,
      ...rowDimensions,
      Population: cells[6] === "" ? null : Number.parseFloat(cells[6]),
      Source: cells[7],
    });
  }
  return selected;
...
function stratifiedRows(rows, {
  ...
  for (const row of base) {
    ...
    if (existing) {
      existing[PARAMETER] =
        existing[PARAMETER] == null || row[PARAMETER] == null
          ? null
          : existing[PARAMETER] + row[PARAMETER];
    } else {
 ...
 * geographic multi-select. Stratification (age / sex / race) and source are
 * deliberately ignored: the list describes the geography, not the slice.
 */
export async function queryLocations({ subset }) {
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  const rows = filterRows(await loadProjectionsData(), { levels });
