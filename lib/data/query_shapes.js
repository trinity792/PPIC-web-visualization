...
/**
 * Distinct location names for the sidebar's geographic multi-select.
 *
 * Deliberately does NOT filter by geographic level: each module resolves its own
 * subset rows first (levels, metro→region aggregation, latest RHNA cycle, …) and
 * passes them in, so this stays a pure "distinct, trimmed, sorted names from one
 * column" shape.
 *
 * @param {Array<object>} rows      rows already narrowed to the requested subset
 * @param {{locationColumn?: string}} [options] the column holding the display
 *   name; defaults to `Location`. RHNA Progress passes `Jurisdiction`.
 * @returns {string[]} locale-sorted unique names; blank and whitespace-only
 *   values are dropped, so the UI never renders an empty checkbox row.
 */
export function buildLocationList(rows, { locationColumn = "Location" } = {}) {
  const names = new Set();
  for (const row of rows || []) {
    const raw = row?.[locationColumn];
    if (raw == null) continue;
    const name = String(raw).trim();
    if (name) names.add(name);
  }
  // Locale sort (not the default code-unit sort) so accented names file next to
  // their unaccented neighbours rather than after "Z".
...
export function buildTwoPeriod(
  ...
  const records = [...byLocation.entries()]
    ...
    .sort((a, b) => {
      const aAvailable = a.start != null && a.end != null;
      const bAvailable = b.start != null && b.end != null;
      if (aAvailable !== bAvailable) return aAvailable ? -1 : 1;
      if (!aAvailable) return a.location.localeCompare(b.location);
      const aChange = a.end - a.start;
      const bChange = b.end - b.start;
