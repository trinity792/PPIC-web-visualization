...
/**
 * Distinct jurisdiction names for one geographic subset, for the sidebar's
 * geographic multi-select. Reads the source column `Jurisdiction` rather than
 * the `Location` alias, and applies only the level filter — income level, cycle,
 * and snapshot recency describe the measurement, not the geography.
 */
export async function queryLocations({ subset }) {
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  const rows = (await loadRhnaProgressData()).filter((row) =>
    levels.includes(row["Geographic Level"]),
  );
  return {
    subset,
    locations: buildLocationList(rows, { locationColumn: "Jurisdiction" }),
  };
}
...
/**
 * Current county standings joined to the canonical county GEOIDs used by the
 * shared California geometry. HCD names county jurisdictions as, for example,
 * "Alameda County", while the GeoJSON display name is "Alameda"; keep the HCD
 * label in the record and normalize only the lookup key.
 */
export async function queryGeoValues(params) {
  ...
  const result = await queryCategoryValues({ ...params, topN: null, sort: "name" });
  const ids = await getFeatureIdLookup("counties");
  const geometryName = (location) => location.replace(/\s+County$/i, "");
  const unmatched = result.records
    .filter((record) => !ids.has(geometryName(record.location)))
    .map((record) => record.location);
  return {
    featureidkey: "properties.GEOID",
    records: result.records
      .filter((record) => ids.has(geometryName(record.location)))
      .map((record) => ({ ...record, geoid: ids.get(geometryName(record.location)) })),
    unmatched,
  };
}
