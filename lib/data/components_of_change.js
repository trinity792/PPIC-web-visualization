...
export const SUBSET_TO_SOURCE = COMPONENTS_OF_CHANGE_SCHEMA.subsetSource;
 ...
 * geographic multi-select. A subset's required source is applied here because
 * Components carries two meanings at the State level: the single DoF
 * California total and the Census national state comparison set.
 */
export async function queryLocations({ subset }) {
  ...
  const rows = filterRows(await loadComponentsOfChangeData(), {
    levels,
    source: SUBSET_TO_SOURCE[subset] || null,
  });
