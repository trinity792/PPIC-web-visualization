 ...
 * geographic multi-select. Source is deliberately ignored: the list describes
 * the geography, not the dataset, so switching DoF ⇄ Census never empties it.
 */
export async function queryLocations({ subset }) {
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  const rows = filterRows(await loadComponentsOfChangeData(), { levels });
