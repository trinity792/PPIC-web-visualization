...
// The legacy sequence-file extract remains the authoritative source for rows
// that the table-based ACS release does not publish. In particular, the 2022
// table release contains the aggregate "All" iteration but no race iterations.
// Current rows still win on an exact contract key so a live vintage is never
// replaced by its legacy counterpart.
const LEGACY_DATA_RELATIVE_PATH = path.join(
  "data",
  "data-cleaned",
  "housing-stress",
  "HousingStress_Current.legacy-2012-2023.csv.bak",
);

const DATA_PATH = path.join(process.cwd(), DATA_RELATIVE_PATH);
const LEGACY_DATA_PATH = path.join(process.cwd(), LEGACY_DATA_RELATIVE_PATH);
const CONTRACT_KEY_COLUMNS = Object.freeze([
  "Year",
  "Geographic Level",
  "Location",
  "Race/Ethnicity",
...
function normalizeLegacyRows(rows) {
  const availablePeriods = new Set(HOUSING_STRESS_SCHEMA.time.availablePeriods);
  return rows
    .filter((row) => availablePeriods.has(row.Year))
    .map((row) => {
      const legacyRace = row["Race/ethnicity"];
      const values = {
        ...row,
        "Race/Ethnicity":
          legacyRace === "American Indian/Alaskan Native" ? "AIAN" : legacyRace,
        Tenure: row.Label,
      };
      return Object.fromEntries(
        HOUSING_STRESS_SCHEMA.canonicalColumns.map((column) => [column, values[column]]),
      );
    });
}

function contractKey(row) {
  return CONTRACT_KEY_COLUMNS.map((column) => row[column]).join("\u001f");
}

function mergeLegacyGaps(legacyRows, currentRows) {
  const byKey = new Map(legacyRows.map((row) => [contractKey(row), row]));
  for (const row of currentRows) byKey.set(contractKey(row), row);
  return [...byKey.values()];
}
...
export async function loadHousingStressData() {
  ...
  let currentText;
  try {
    currentText = await readFile(DATA_PATH, "utf8");
  } catch (error) {
  ...
  let legacyText = null;
  try {
    legacyText = await readFile(LEGACY_DATA_PATH, "utf8");
  } catch (error) {
    if (error.code !== "ENOENT") throw error;
  }

  cachedRows = legacyText
    ? mergeLegacyGaps(normalizeLegacyRows(parseCsv(legacyText)), currentRows)
    : currentRows;
  return cachedRows;
}
 ...
 * geographic multi-select. Race/Ethnicity and Tenure are deliberately ignored:
 * the list describes the geography, not the stratification slice.
 */
export async function queryLocations({ subset }) {
  const levels = SUBSET_TO_LEVELS[subset];
  if (!levels) throw new Error(`Unknown subset: ${subset}`);
  const rows = filterRows(await loadHousingStressData(), { levels });
...
export async function queryFullTable({
  ...
  const rows = await loadHousingStressData();
