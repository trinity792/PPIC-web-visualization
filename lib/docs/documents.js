 ...
 * frontmatter (Topic, Content Type, Status, description, dates, Footnote),
 ...
 *
 * Outputs:
 *   - getDocuments()    — normalized, non-archived doc records (newest first)
 *   - getContentTypes() — sorted unique Content Type values
 *   - getStatuses()     — ["All statuses", ...unique statuses]
 ...
 *   - Curation excludes archived documents, index/landing pages, empty
 *     placeholders, and any file missing a Content Type (e.g. the untracked
 *     trinitys_notes/primary.md).
 */
...
function loadDocuments() {
  ...
  for (const absPath of walkMarkdown(DOCS_ROOT)) {
    ...
    const status = typeof fm.Status === "string" ? fm.Status.trim() : null;

    // Curation: real documents only.
    if (!type || EXCLUDED_TYPES.has(type.toLowerCase())) continue;
    if (status?.toLowerCase() === "archive") continue;
    if (!body) continue;
    ...
    docs.push({
      ...
      footnote: fm.Footnote ? String(fm.Footnote).trim() : "",
...
export function getStatuses() {
  const statuses = [
    ...new Set(loadDocuments().map((d) => d.status).filter(Boolean)),
