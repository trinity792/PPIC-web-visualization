 ...
 *   inlinePeriods(table, spec)  — the ascending periods a time-axis shape plots,
 *                                the base-period choices for base-year change
...
const LEGACY_SHAPE_BY_CHART = Object.freeze({
  line: "line",
  bar: "category",
  pie: "category",
  dumbbell: "twoPeriod",
...
/**
 * The distinct periods an inline chart's time axis actually plots, ascending.
 *
 * Only the line and matrix shapes read a time axis (`bindings.x`); every other
 * shape tags each row with one implied period (see `IMPLIED_PERIOD` above), so
 * there is no base period to index against and the list is empty. Periods are
 * parsed with the same `numberCell` the builders use, so a period offered here
 * is always identical to a `years` entry the transform registry will look up.
 * Non-numeric period labels (a matrix's raw text x-axis) are left out: a base
 * period is a number by contract, and the registry already falls back to each
 * series' first value when it cannot find one.
 */
export function inlinePeriods(table, spec) {
  const shape = LEGACY_SHAPE_BY_CHART[spec?.chartType];
  if (shape !== "line" && shape !== "matrix") return [];
  const filtered = applyTabFilter(table, spec?.filters);
  const index = columnIndex(filtered, spec?.bindings?.x);
  if (index === -1) return [];
  const periods = new Set();
  for (const row of filtered?.rows || []) {
    const period = numberCell(row?.[index]);
...
/**
 * Can a pasted table feed this chart type at all? `buildShapes` answers
 * `{ records: [] }` for a chart type with no shape of its own, which reads to
 * the reader as an empty chart rather than as an unsupported one — so the
 * chart-type galleries ask this first and do not offer it (today: symbolMap,
 * which has no inline coordinate contract; see lib/visualization/chartAvailability.js).
 */
export function hasInlineShape(chartTypeId) {
  const shape = LEGACY_SHAPE_BY_CHART[chartTypeId];
  return Boolean(shape && SHAPE_BUILDERS[shape]);
}
...
export function supportedShapes(table) {
  ...
  if (dimensionCols >= 1 && numberCols >= 2) supported.push("dumbbell");
