...
/** Return one change observation for every adjacent ordered pair. */
function adjacentChange(series, calculationId) {
  const years = [];
  const values = [];
...
export const TRANSFORMS = Object.freeze({
  ...
  percentChange: (series) => adjacentChange(series, "percentChange"),

  /** Point change from each preceding period. For rate/percentage fields. */
  percentagePointChange: (series) =>
    adjacentChange(series, "percentagePointChange"),
