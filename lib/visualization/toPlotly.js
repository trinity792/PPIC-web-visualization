...
function twoPeriodSpec(spec, { slope = false, forest = false } = {}) {
  ...
  // study, style-controlled endpoint caps, an estimate marker (optionally sized
  // by study weight), and a "line of no effect".
  if (forest) {
    ...
    const weights = records.map((row) => valueOf(row, bindings.size, ["size", "weight"]));
    const finiteWeights = weights.map(Number).filter(Number.isFinite);
    const hasWeight = Boolean(bindings.size) && finiteWeights.length > 0;
    const wMin = hasWeight ? Math.min(...finiteWeights) : 0;
    const wMax = hasWeight ? Math.max(...finiteWeights) : 0;
    // Study weight → marker size (bigger = more precise); flat size otherwise.
    const sizeForWeight = (weight) => {
      const value = Number(weight);
      if (!hasWeight || !Number.isFinite(value) || wMax === wMin) return 16;
      return 10 + ((value - wMin) / (wMax - wMin)) * 20; // 10–30 px
    };
        ...
        ? [
            {
              ...
              marker: {
                symbol: pointSymbol,
                color: markerColor,
                size: hasWeight ? weights.map(sizeForWeight) : 13,
...
export function toPlotly(spec) {
  switch (spec.chartType) {
    ...
    case "bar":
      return barSpec(spec);
    case "divergingBar":
      return divergingBarSpec(spec);
    case "dumbbell":
      return twoPeriodSpec(spec, {});
    case "slope":
      return twoPeriodSpec(spec, { slope: true });
    case "forest":
      return twoPeriodSpec(spec, { forest: true });
    case "dotPlot":
