...
// Sequential choropleth ramp: light blue (low) → dark blue (high).
const CHOROPLETH_BLUES = Object.freeze([
  [0, COLORS.blue1],
  [1, COLORS.blue5],
]);
...
function legendFor(position = "right") {
  ...
  if (position === "bottom") {
    return {
      showlegend: true,
      // Anchor the legend's top below the x-axis title so they don't overlap;
      // baseLayout reserves extra bottom margin to match.
...
function baseLayout(labels = {}, appearance = {}) {
  ...
  return {
    ...
    margin: {
      ...
      b: appearance.legendPosition === "bottom" ? 104 : 60,
    },
    autosize: true,
    ...legend,
    ...
    data: [
      {
        ...
        colorscale:
          appearance.colorScale === "diverging" ? "RdBu" : CHOROPLETH_BLUES,
        marker: {
          line: {
            color: COLORS.white,
