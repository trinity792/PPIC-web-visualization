...
import { paletteForScale, resolveToken, seriesColor } from "./palettes";
...
function measureAxes(spec) {
  ...
  // Workstream B: a diverging bar — the retired `divergingBar` id, or a plain
  // `bar` with `appearance.diverging` on — reads its measure axis with the
  // orientations reversed from a plain bar's, preserving each variant's
  // pre-merge default exactly.
  const diverging = chartType === "divergingBar" || Boolean(appearance.diverging);
  switch (chartType) {
    case "line":
      return ["yaxis"];
    case "bar":
...
function withGroupedCategoryAxis(layout, sections, axisName, appearance = {}) {
  ...
  return {
    ...
    margin: {
      ...layout.margin,
      ...(axisName === "x" ? { t: (layout.margin?.t || 0) + axisFontSize + 12 } : {}),
    },
    annotations: [...(layout.annotations || []), ...headers],
...
function twoPeriodSpec(spec, { slope = false, forest = false } = {}) {
      ...
      return {
        ...
        ...(sections.grouped
          ? {
              legendgroup: group,
              ...(firstInGroup && group
                ? { legendgrouptitle: { text: `<b>${escapeLegendText(group)}</b>` } }
                : {}),
            }
          : {}),
  ...
  if (forest) {
    ...
    const visualRows = sections.grouped ? sections.maxPosition + 2 : records.length;
    forestLayout.height = Math.max(520, 220 + visualRows * 52);
    ...
    data: [
      {
        ...
        marker: {
          ...
          // One colour for every marker, taken from the active palette rather
          // than hard-coded, so the Color Palette control actually reaches this
          // chart. Keyed on the plotted measure so a per-series override in
          // `appearance.seriesColors` can name it, the same way the forest
          // plot keys its single marker colour.
...
function linePositionCounts(chartType, data) {
  ...
  if (chartType === "bar" || chartType === "divergingBar") {
    // Already covers a diverging `bar` (appearance.diverging) with no change:
    // the check is on `chartType`, which stays "bar" either way.
...
export function toPlotly(spec) {
  ...
  switch (spec.chartType) {
    ...
    case "bar":
      ...
      // same renderer the retired `divergingBar` id used, rather than a
      // separate chart type. `case "divergingBar"` below keeps falling into
      // the same expression until Phase 4 (one release after the migration).
  ...
  return withHiddenSeries(withLinePaddingMeta(withMeasureFormat(result, spec), spec), spec);
}
