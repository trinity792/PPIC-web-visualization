...
function decimalPlaces(spec) {
  const raw = Number(spec?.appearance?.decimalPlaces);
  return Number.isFinite(raw) && raw >= 0
    ? Math.min(10, Math.trunc(raw))
...
function axisNumberFormat(spec, axis) {
  const key = axis === "xaxis" ? "horizontalNumberType" : "verticalNumberType";
  const type = spec?.appearance?.[key];
  if (!NUMBER_TYPES.has(type)) {
    return {
      d3: measureFormat(spec),
      prefix: "",
      // Year-over-year values are already percentage points (10 means 10%),
      // so Plotly must append a percent sign rather than applying d3's `%`
      // formatter, which would multiply the value by 100 a second time.
      suffix: spec?.transforms?.id === "percentChange" ? "%" : "",
      explicit: false,
    };
  }
  const d3 = `,.${decimalPlaces(spec)}f`;
  return {
    d3,
    prefix: type === "usd" ? "$" : "",
    suffix: type === "percent" ? "%" : "",
...
function formattedAxisToken(token, spec, axis, fallbackFormat = null) {
  const numberFormat = axisNumberFormat(spec, axis);
  const formatted = numberFormat.d3
    ? fmtToken(token, numberFormat.d3)
    : fallbackFormat
      ? fmtToken(token, fallbackFormat)
      : token;
...
function withMeasureFormat(result, spec) {
  ...
  for (const axis of axes) {
    const numberFormat = axisNumberFormat(spec, axis);
    if (!numberFormat.d3 && !numberFormat.prefix && !numberFormat.suffix) continue;
    changed = true;
    layout[axis] = {
      ...(layout[axis] || {}),
      ...(numberFormat.d3
        ? { hoverformat: numberFormat.d3, tickformat: numberFormat.d3 }
        : {}),
      ...(numberFormat.prefix ? { tickprefix: numberFormat.prefix } : {}),
      ...(numberFormat.suffix ? { ticksuffix: numberFormat.suffix } : {}),
    };
  }
  return changed ? { ...result, layout } : result;
}

const MANUAL_TICK_INCREMENTS = new Set([1, 2, 5, 10]);

/** One of the editor's safe data-unit intervals, or undefined for auto ticks. */
function tickIncrement(value) {
  const number = Number(value);
  return MANUAL_TICK_INCREMENTS.has(number) ? number : undefined;
}

const MAX_MANUAL_TICK_RANGE = 75;

function finiteTickValue(value) {
  if (value == null || value === "") return null;
  const number = Number(value);
  return Number.isFinite(number) ? number : null;
}

function renderedAxisSpan(result, axis) {
  const layoutRange = result.layout?.[`${axis}axis`]?.range;
  if (Array.isArray(layoutRange) && layoutRange.length >= 2) {
    const [start, end] = layoutRange.map(Number);
    if (Number.isFinite(start) && Number.isFinite(end)) {
      return Math.abs(end - start);
    }
  }

  const finite = (result.data || [])
    .flatMap((trace) => {
      const values = Array.isArray(trace?.[axis]) ? trace[axis] : [];
      const barMeasureAxis =
        trace?.type === "bar" &&
        ((trace.orientation === "h" && axis === "x") ||
          (trace.orientation !== "h" && axis === "y"));
      if (!barMeasureAxis) return values;

      const bases = Array.isArray(trace.base)
        ? trace.base
        : values.map(() => finiteTickValue(trace.base) ?? 0);
      return values.flatMap((value, index) => {
        const number = Number(value);
        const base = finiteTickValue(bases[index]) ?? 0;
        return Number.isFinite(number) ? [base, base + number] : [base];
      });
    })
...
/** Apply the editor's physical horizontal/vertical increments to x/y axes. */
function withTickIncrements(result, spec) {
  if (!result?.layout) return result;
  const horizontalSpan = renderedAxisSpan(result, "x");
  const verticalSpan = renderedAxisSpan(result, "y");
  const horizontal =
    horizontalSpan != null && horizontalSpan <= MAX_MANUAL_TICK_RANGE
      ? tickIncrement(spec.appearance?.horizontalTickIncrement)
      : undefined;
  const vertical =
    verticalSpan != null && verticalSpan <= MAX_MANUAL_TICK_RANGE
      ? tickIncrement(spec.appearance?.verticalTickIncrement)
      : undefined;
  if (horizontal == null && vertical == null) return result;
  ...
  return {
    ...
    layout: {
      ...result.layout,
      ...(horizontal != null
        ? { xaxis: { ...(result.layout.xaxis || {}), dtick: horizontal } }
        : {}),
      ...(vertical != null
        ? { yaxis: { ...(result.layout.yaxis || {}), dtick: vertical } }
        : {}),
    },
  };
}
...
function lineSpec(spec) {
  ...
  const data = transformed.series.map((item, index) => {
    ...
    return {
      ...
      hovertemplate:
        labels.tooltip ||
        `%{x}<br>${formattedAxisToken("%{y}", spec, "yaxis")}<extra>${name}</extra>`,
    };
  });
...
function barSpec(spec) {
  ...
  const data = [...groups.entries()].map(([group, rows], index) => {
    ...
    return {
      ...
      texttemplate: appearance.showValueLabels
        ? formattedAxisToken("%{text}", spec, measureAxis)
        : undefined,
      ...
      hovertemplate:
        ...
        (sections.grouped
          ? horizontal
            ? `%{customdata}: ${formattedAxisToken("%{x}", spec, measureAxis)}<extra></extra>`
            : `%{customdata}: ${formattedAxisToken("%{y}", spec, measureAxis)}<extra></extra>`
          : horizontal
            ? `%{y}, ${formattedAxisToken("%{x}", spec, measureAxis)}<extra></extra>`
            : `%{x}, ${formattedAxisToken("%{y}", spec, measureAxis)}<extra></extra>`),
    };
  });
  const layout = {
...
function divergingBarSpec(spec) {
  ...
  const trace = {
    ...
    hovertemplate:
      labels.tooltip ||
      (sections.grouped
        ? `%{customdata[1]}<br>${formattedAxisToken("%{customdata[0]}", spec, measureAxis)}<extra></extra>`
        : horizontal
          ? `%{y}<br>${formattedAxisToken("%{customdata}", spec, measureAxis)}<extra></extra>`
          : `%{x}<br>${formattedAxisToken("%{customdata}", spec, measureAxis)}<extra></extra>`),
  };
...
function centeredForestRange(values, rawCenter) {
  if (rawCenter == null || rawCenter === "") return null;
  ...
  const finiteValues = values
    ...
    .map(Number)
    .filter(Number.isFinite);
  const dataRadius = finiteValues.length
    ? Math.max(...finiteValues.map((value) => Math.abs(value - center)))
    : 0;
  // A small non-zero radius keeps an all-center dataset visible. Otherwise,
  // add 5% breathing room so the furthest marker is not clipped at the frame.
  const radius = dataRadius || Math.max(Math.abs(center) * 0.05, 0.05);
...
function twoPeriodSpec(spec, { slope = false, forest = false } = {}) {
  ...
  const pointLabelTemplate = formattedAxisToken("%{x}", spec, "xaxis", ",");
  ...
  if (forest) {
    ...
    const estimateName = String(bindings.point || "Estimate");
    ...
    const ciLines = records.map((_, index) => ({
      ...
      customdata: [
        [starts[index], ends[index], categories[index]],
        [starts[index], ends[index], categories[index]],
      ],
      showlegend: false,
      hovertemplate:
        labels.tooltip ||
        `${sections.grouped ? "%{customdata[2]}" : "%{y}"}<br>${startName}: ${fmtToken("%{customdata[0]}", format)}<br>${endName}: ${fmtToken("%{customdata[1]}", format)}<extra></extra>`,
    }));

    const endpointStyle = appearance.endpointStyle || "caps";
    const endpointSymbol = FOREST_ENDPOINT_SYMBOL[endpointStyle];
    const endpointUsesPalette =
      endpointStyle !== "none" && endpointSymbol !== "line-ns";
    const endpointColor = (name, index) =>
      endpointUsesPalette || appearance.seriesColors?.[name]
        ? seriesColor(appearance, name, index)
        : COLORS.gray5;
    const endpointTrace = (name, values, index) => {
      const color = endpointColor(name, index);
      return {
        type: "scatter",
        mode: "markers",
        ...
        hovertemplate:
          labels.tooltip ||
          `${sections.grouped ? "%{customdata}" : "%{y}"}<br>${name}: ${fmtToken("%{x}", format)}<extra></extra>`,
      };
    };
    const endpointTraces =
      endpointStyle !== "none" && endpointSymbol
        ? [
            endpointTrace(startName, starts, 0),
            endpointTrace(endName, ends, 1),
          ]
        : [];
        ...
        ? [
            {
              ...
              name: wrapLegendLabel(estimateName, appearance),
              x: estimates,
              y: axisCategories,
              ...
              marker: {
                ...
                color: seriesColor(
                  appearance,
                  estimateName,
                  endpointUsesPalette ? 2 : 0,
                ),
                size: pointSymbol === "circle" ? 8 : 13,
    ...
    const centeredRange = centeredForestRange(
      [...starts, ...ends, ...estimates, noEffect],
      appearance.center,
    );
    if (centeredRange) {
      forestLayout.xaxis = {
...
function heatmapSpec(spec) {
  ...
  // Apply the active time-series transform one row (place) at a time: each row
  // becomes a {years, values} series so the shared, null-safe transformRegistry
  // can run unchanged (flagged issue 1). "actual" (the default) is a no-op, so
  // untransformed heatmaps render unchanged.
  ...
  const x = transformedRows.series[0]?.years || matrix.x;
...
export function toPlotly(spec) {
  ...
  const formatted = withLinePaddingMeta(
    withTickIncrements(withMeasureFormat(result, spec), spec),
    spec,
  );
