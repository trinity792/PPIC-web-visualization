...
import { hasComparisonDimensions } from "@/lib/visualization/comparisons";
...
export const SIDEBAR_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    when: (config, schema) =>
      config.version === 3 && hasComparisonDimensions(schema),
  },
  {
    value: "time",
    label: "Time",
    Component: TimeSection,
    // A chart with no time axis (a forest plot; pasted data with no time
    // column) has nothing to ask, so the heading goes too.
    when: (config) => config.version === 3 && config.question?.time?.contract !== "none",
  },
  {
    value: "geography",
...
export function visibleSectionsFor(config, schema, { only, exclude } = {}) {
  ...
  const chartSections = getChartType(chartType)?.sidebarSections || [];
