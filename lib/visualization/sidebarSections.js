...
export const SIDEBAR_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    // Deliberately no `key`. "comparison" was the wrong gate twice over: nearly
    // every chart type declares it, including the ones that are not
    // `transformCapable`, so a Range chart kept a "Transform" heading with
    // nothing under it; and the stratification pins this section now owns are a
    // statement about rows, not about the chart, so they must survive on the
    // chart types that declare no comparison at all. The section's own render
    // decision answers both.
    when: hasTransformControls,
  },
  {
    value: "labels",
    label: "Labels",
