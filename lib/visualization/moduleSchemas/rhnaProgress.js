...
export const RHNA_PROGRESS_SCHEMA = Object.freeze({
  ...
  // The county subset can also join to the shared California county geometry.
  defaultPreset: "compare-places",
  // Ranking bars, including the diverging variant (each jurisdiction's pace
  // score read against a reference line) via `appearance.diverging` on `bar`
  // (Workstream B) rather than a separate id, plus county choropleths. Trend
  // lines and other chart types stay deferred.
  supportedChartTypes: ["bar", "choroplethMap"],
  // v3: Income Level is a real comparison dimension (mirrors Housing Stress'
  // Race/Ethnicity and Tenure).
  comparisonDimensions: Object.freeze([
    Object.freeze({ id: "Income Level", values: INCOME_LEVELS }),
  ]),
  labelMeta: Object.freeze({
    dimensionOrder: Object.freeze(["geography", "Income Level"]),
    omitValues: Object.freeze({ "Income Level": Object.freeze(["Total"]) }),
    disambiguateBy: Object.freeze(["geography"]),
  }),
  // v3 time: deliberately no `availablePeriods` list. The dataset is
  // snapshot-versioned (biweekly HCD refresh) with no clean, stable public
  // list of past snapshot dates yet — the same reason `hasTemporalData` stays
  // false and the sidebar hides temporal controls in v2. `executeQuestion.js`
  // resolves the server-side "snapshot" contract to the adapter's own
  // `defaultReportingPeriod()` (the latest row in the live table) even
  // without a static list here, so requests still work; `questionReadiness.js`
  // and the Time section read the empty list as "nothing to pick" rather than
  // an unfinished selection, so only a manual snapshot picker is unavailable,
  // matching current behavior. Revisit once the snapshot history has a
  // stable, developer-approved list to publish.
  time: Object.freeze({
    availablePeriods: Object.freeze([]),
    reportingPeriods: Object.freeze([]),
    defaultReportingPeriod: null,
  }),
});
