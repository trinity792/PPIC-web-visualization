 ...
 * `docs/trinitys_notes/main.md`. Canonical column names match the
 ...
 *
 * Towns (DoF's 22 incorporated towns, e.g. Paradise, Los Gatos, Truckee) are
 * listed under Cities: readers look for them there, and a separate "Towns"
 * level hid them. The rows keep their `Town` Geographic Level in the data.
 */
...
export const POPHOUSING_SCHEMA = Object.freeze({
  ...
  sources: null, // Source is per-row provenance here, not a dataset toggle
  // `Source` (E-5 / E-8 / Aggregated) is provenance carried on each row rather
  // than a mutually-exclusive dataset (B3), so `filters.source` takes an array
  // shape for this module and `chartData.js` reads this flag to know that.
  //
  // It no longer draws a control. The E-5/E-8/Aggregated multi-select was the
  // only thing in this module's "Datasets" block, on a module with exactly one
  // dataset, and it defaulted to every vintage - a heading that promised a
  // dataset choice and delivered a provenance filter nobody needed to touch.
  // Omitting the filter is what the API already reads as "all vintages", so
  // removing the control changes no chart.
  provenanceFilter: true,
  // Declares that a server-side refresh pipeline exists (POST {apiPath}/update).
  // The "Update data" button that fired it is unwired pending the refresh
  // overhaul, so nothing in the editor reads this today.
  refreshable: true,
  yearRange: [1991, 2026], // cleaned E-5 coverage (through the latest DoF vintage)
  // v3: comparisons here are location-only, like Components of Change — there is
  // no extra stratification dimension to build a comparison card from.
  ...
  time: Object.freeze({
    availablePeriods: Object.freeze(
      Array.from({ length: 2026 - 1991 + 1 }, (_, index) => 1991 + index),
    ),
    reportingPeriods: Object.freeze([2026]),
    defaultReportingPeriod: 2026,
  }),
});
