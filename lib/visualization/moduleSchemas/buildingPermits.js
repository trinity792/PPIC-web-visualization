...
// Counts of authorized units; base-year and two-period change are stock-style
...
export const BUILDING_PERMITS_SUBSETS = Object.freeze({
  ...
  // 9 CA regions: metros aggregated on demand in the data layer, which stamps
  // the derived rows "Region" so the v3 adapter can filter on this level.
  Regions: ["Region"],
...
/**
 * v3 monthly period tokens, "YYYY-MM", inclusive of both ends. Periods are
 * opaque, lexicographically-sortable strings to the v3 time engine (see
 * `executeQuestion.js` `comparePeriods`), so month grain works the same way
 * year grain does elsewhere — only the token shape differs.
 */
function monthlyPeriods(startYearMonth, endYearMonth) {
...
export const BUILDING_PERMITS_SCHEMA = Object.freeze({
  ...
  yearRange: [2010, 2026],
  // v3: comparisons here are location-only, like Components of Change — no
  // extra stratification dimension exists to build a comparison card from.
  comparisonDimensions: Object.freeze([]),
  labelMeta: Object.freeze({
    dimensionOrder: Object.freeze(["geography"]),
    omitValues: Object.freeze({}),
    disambiguateBy: Object.freeze(["geography", "time"]),
  }),
  time: Object.freeze({
    // Bump the end token alongside yearRange as new Census BPS months land.
    availablePeriods: Object.freeze(monthlyPeriods("2010-01", "2026-06")),
    reportingPeriods: Object.freeze(["2026-06"]),
    defaultReportingPeriod: "2026-06",
  }),
  // Module-owned presets (graph-editor overhaul, Phase 6). Their presence, plus
  // the monthly temporal control, is what lifts this module out of the
  // UnderConstruction placeholder — so `underConstruction` is no longer set.
  presets: [
    {
      id: "permits-monthly-trend",
      title: "Monthly permit trend",
      question: "How have authorized units trended month over month?",
      config: {
        module: "building-permits",
        preset: "trend-over-time",
        chartType: "line",
        bindings: { x: "Date", y: "Total" },
        filters: { subset: "Metros" },
        period: {},
        transform: "actual",
        comparisonMode: "places",
        layers: [],
        labels: { title: "Authorized housing units by month" },
        appearance: {},
        data: { source: "module" },
      },
    },
    {
      id: "permits-multifamily-change",
      title: "Multifamily permit change",
      question: "How did multifamily permits shift between two periods by metro?",
      // twoPeriod shape — one of the views /api/building-permits serves (line,
      // twoPeriod, geoValues). The category/bar view is not wired to this
      // module's API yet, so its presets stay on the supported shapes.
      config: {
        module: "building-permits",
        preset: "two-period-gap",
        chartType: "dumbbell",
        bindings: { category: "Location", start: "2+ Units", end: "2+ Units" },
        filters: { subset: "Metros" },
        period: {},
        transform: "actual",
        comparisonMode: "places",
        layers: [],
        labels: { title: "Multifamily (2+ unit) permit change by metro" },
        appearance: {},
        data: { source: "module" },
      },
    },
