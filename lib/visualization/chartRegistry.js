...
export const SKELETON_SHAPES = Object.freeze([
  "bars", // vertical columns (bar/divergingBar at their vertical default)
  "barsHorizontal", // horizontal bars (bar/divergingBar flipped by orientation)
...
export const CHART_TYPES = Object.freeze({
  line: {
    ...
    requiredRoles: ["x", "y"],
    optionalRoles: ["series", "color", "benchmark"],
  ...
  bar: {
    ...
    defaults: {
      ...
      // already varies through `hole`; see the retired `divergingBar` id in
      // chartSpec.js's RETIRED_CHART_TYPES.
      diverging: false,
  ...
  divergingBar: {
    id: "divergingBar",
    // A Bar-family variant: same category+measure contract as `bar`, but each
    // bar is drawn from a center reference value so categories above the
    // reference extend one way and those below extend the other (e.g. RHNA
    // on-track scores diverging from 1.0 = "on pace"). Renders through the same
    // bar path in toPlotly with a `center` offset instead of a zero baseline.
    //
    // RETIRED (Workstream B, 2026-07-30): superseded by `bar` + `appearance.
    // diverging`. Kept registered for one more release so a config that has
    // not yet passed through normalizeSpec's `RETIRED_CHART_TYPES` rewrite
    // (chartSpec.js) still resolves everywhere `getChartType("divergingBar")`
    // is read defensively; `hidden: true` only pulls it out of the chart-type
    // grid (ChartTypeSection.js's `orderedChartTypes`). Delete outright one
    // release after the migration ships (Phase 4).
    hidden: true,
    label: "Diverging Bar",
    purpose: "Rank categories above and below a reference value, diverging from a center line.",
    transformCapable: true,
    rankingCapable: true,
    lineAxes: ["horizontal", "vertical"],
    requiredRoles: ["category", "y"],
    ...
    // Same category-is-geography inference as `bar` (Workstream A).
    impliedRoles: { category: "geography" },
    // Same shape family as `bar`; its horizontal default flips to
    // "barsHorizontal" via the same `skeletonShapeFor` orientation check.
    skeletonShape: "bars",
    sidebarSections: ["data", "encodings", "comparison", "labels", "appearance"],
    defaults: {
      // Diverging bars read best horizontally (one long axis for the divergence).
      orientation: "horizontal",
      // The value the bars diverge from; 0 unless the measure has a meaningful
      // midpoint (e.g. 1.0 for a pace ratio, a survey-neutral point).
      center: 0,
      sort: "value",
      showValueLabels: false,
      groupGap: 0.75,
    },
    limits: { recommendTopN: 20 },
  },
  ...
  dumbbell: {
    ...
    optionalRoles: ["point", "size", "benchmark", "group"],
    hiddenRoles: ["size"],
    roleConstraints: {
      category: [DIMENSION],
      start: [MEASURE],
