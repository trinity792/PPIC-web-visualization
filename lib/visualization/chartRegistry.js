...
export const CHART_TYPES = Object.freeze({
  ...
  forest: {
    ...
    // / `end` = the CI's lower / upper bound, `point` = the study estimate, `size`
    // = the study weight (bigger marker = more precise). start/end are DISTINCT
    // columns (a CI), so endpoints are not mirrored the way a two-period dumbbell's
    // are.
    requiredRoles: ["category", "start", "end"],
    optionalRoles: ["point", "size", "benchmark"],
    roleConstraints: {
      category: [DIMENSION],
      start: [MEASURE],
      end: [MEASURE],
    ...
    controlTiers: {
      showValueAxis: "moderate",
      showPointLabels: "moderate",
      endpointStyle: "moderate",
      pointStyle: "moderate",
      noEffectValue: "moderate",
    },
