...
export const TREND_OVER_TIME = Object.freeze({
  ...
  requiredRoles: ["x", "y"],
  // Workstream D: `benchmark` role deleted — nothing ever read it.
  optionalRoles: ["series"],
  defaults: {
    x: { role: "temporal" },
    y: { role: "measure" },
  ...
  sidebar: {
    ...
    comparison: ["locations", "baseYear", "transform"],
...
export const DIVERGING_RANKING = Object.freeze({
  ...
  // A Bar variant since Workstream B, not a separate chart type: `diverging`
  // routes through the same category/measure contract `compare-places` uses.
  chartType: "bar",
...
// DIVERGING PACE (dashboard style) — the same diverging ranking rendered to
// mirror the landing on-track bars: a fixed value scale, a background track
// rail, threshold ("traffic-light") bucket colors, minimal axis chrome, and a
// pace center of 1.0. Tuned for pace/ratio scores where 1.0 = on target.
export const DIVERGING_PACE = Object.freeze({
  id: "diverging-pace",
  // A Bar variant since Workstream B, not a separate chart type (see
  // DIVERGING_RANKING above).
  chartType: "bar",
  title: "Pace ranking (dashboard style)",
  question: "Which places are keeping pace with a target?",
  requiredRoles: ["category", "y"],
  optionalRoles: ["color"],
  defaults: {
    category: { role: "comparisonDimension", default: "Location" },
...
export const COMPOSITION = Object.freeze({
  ...
  requiredRoles: ["category", "y"],
  // Slices already take their palette colour from their own category label, so
  // the chart type declares no `color` role and neither does this preset.
  optionalRoles: [],
  defaults: { hole: 0 },
  sidebar: {
    data: true,
...
export const SYMBOL_MAP = Object.freeze({
  ...
  // Every marker shares one palette colour, so there is no `color` role to bind.
  optionalRoles: [],
  defaults: { sizeByArea: true },
  sidebar: {
    data: true,
    encodings: ["geography", "size"],
    labels: ["title", "subtitle", "tooltip"],
    appearance: ["opacity", "legendPosition"],
  },
...
export const PRESETS = Object.freeze({
  ...
  [DIVERGING_PACE.id]: DIVERGING_PACE,
