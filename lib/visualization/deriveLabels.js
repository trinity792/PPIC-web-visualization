...
function axisBindingNames(chartType, bindings, appearance) {
  switch (chartType) {
    case "bar":
    case "divergingBar": {
      // Diverging bars default to horizontal; both share the category/measure
      // axis mapping, flipped by orientation. `appearance.diverging` carries
      // the default since Workstream B retired the separate `divergingBar` id
      // in favor of a `bar` variant flag; the id is kept alongside it only for
      // a config that has not yet passed through normalizeSpec's rewrite.
