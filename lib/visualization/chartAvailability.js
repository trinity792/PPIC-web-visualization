 ...
 * allowlist, the `hidden` marker on a retired descriptor, and — for the two map
 ...
 * much they exclude:
 *
 *   1. A `hidden` descriptor is never offered (the retired `divergingBar` id,
 *      kept registered so a bookmarked link still resolves).
...
export function isChartTypeAvailable(chartTypeId, schema) {
  ...
  if (!chart || chart.hidden) return false;
