...
import { impliedBindings } from "./impliedRoles";
...
export function validateBindings(chartTypeId, bindings = {}, schema, { schemaFields = true } = {}) {
  ...
  for (const role of chart.requiredRoles) {
    // Declared implied AND resolved by the schema: not asked of the reader, so
    // its absence from `bindings` is not a fault. A chart type that declares a
    // role implied but sits on a schema that cannot resolve it (checked via
    // `implied[role]`, not `isImpliedRole`) still reports it missing — there it
    // really is unset.
    if (implied[role]) continue;
    if (!bindings[role]) {
      findings.push({
  ...
  for (const [role, fieldName] of schemaFields ? Object.entries(bindings) : []) {
    ...
    // Workstream D: the per-field catalog-role curation (`chartRoles`) is now
    // an Advanced Mode escape hatch, not a wall — a reader who binds a field
    // the catalog doesn't endorse gets told why the curation existed rather
    // than being blocked outright. `hasBlockingErrors` must not see this: it
    ...
    if (
      ...
      findings.push(
        warn(
          "ROLE_NOT_IN_CATALOG",
          `"${fieldName}" is not curated for use as "${role}".`,
          "This field isn't listed for this encoding in the module's catalog, but it will still render.",
        ),
      );
    }
  }
 ...
 *
 * Imported data is exempt: the window is the module year slider's, and an inline
 * chart plots every row it was given regardless (lib/tabular/toSeries.js passes
 * the period only as a fallback range). Its base period is picked from the
 * imported column's own values, so comparing the two would warn about a mismatch
 * that does not exist — a table of months indexed to month 3 is not "outside
 * 1990–2026".
 *
 * @param {object} config { transform, period, data }
 */
export function validateBaseYear(config) {
  const transform = config?.transform;
  if (!BASE_YEAR_TRANSFORMS.has(transform)) return [];
  if (config?.data?.source === "inline") return [];
 ...
 * @param {object} [opts]  { seriesCount, geoUnmatched, presets }
 * @param {boolean} [opts.presets] default true. The module workbench has no
 *   preset picker and never binds a field on the reader's behalf (Workstream
 *   E), so it passes `false` here to skip `validatePresetBindings` — a config
 *   still carries `preset` for bookkeeping (SET_CHART_TYPE), but a surface
 *   that never shows a preset must not be told off for not satisfying one.
 *   `chartSpec.js` and `savedViews.js`, which validate configs outside any
 *   surface, keep the default (true).
 */
export function validateConfig(config, schema, opts = {}) {
  ...
  const presets = opts.presets !== false;
  ...
  const findings = [
    ...
    ...(presets ? validatePresetBindings(config.preset, bindings) : []),
    ...(chart && !isInline
...
/**
 * The findings that mean "this setting has not been chosen yet" rather than
 * "this setting is wrong".
 *
 * They are still errors — a chart missing its y-axis cannot be drawn or saved —
 * but on a surface that binds nothing on the reader's behalf (the module
 * workbench, `autoBind: false`) an unbound required role is the normal opening
 * state of every chart, not a fault to report. Those surfaces show the skeleton
 * and stay quiet; every other surface keeps treating them as errors.
 */
export const INCOMPLETE_CODES = Object.freeze([
  "MISSING_REQUIRED_ROLE",
  "MISSING_PRESET_ROLE",
]);

/** Whether a finding list says the chart is merely unfinished. */
export function isIncomplete(findings = []) {
  return findings.some((f) => INCOMPLETE_CODES.includes(f.code));
}

/**
 * The roles a finding list reports as unset, in first-seen order — what the
 * skeleton names back to the reader ("Set X-Axis and Y-Axis to build this
 * chart").
 */
export function unsetRoles(findings = []) {
  const roles = [];
  for (const finding of findings) {
    if (!INCOMPLETE_CODES.includes(finding.code)) continue;
    if (finding.role && !roles.includes(finding.role)) roles.push(finding.role);
  }
  return roles;
}
