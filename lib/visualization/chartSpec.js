 ...
 */
// Workstream D: `benchmark` was a `filters.benchmark` value written by the
// now-deleted role's editor control (TransformSection.js's "Benchmark label"
// input). Listed here so a stored view that still carries it doesn't warn
// SPEC_UNKNOWN_KEY; the differenceFromBenchmark transform's `benchmark`
// trace-layer mechanism is unrelated and untouched.
const RETIRED_KEYS = Object.freeze(["tier", "benchmark"]);

/**
 * Runtime-only keys: never serialized, never diffed. `geoUnmatched` (location
 * names a geographic join couldn't match), `seriesNames` (the last-loaded
 * trace names), `legendNames` (the discrete legend entries), and
 * `categoryNames` (the last-loaded line/bar labels), and `axisRanges` (numeric
 * extents used to guard tick increments) are populated by SET_SERIES_COUNT
 * alongside `seriesCount` — all are load-derived, not user configuration.
 */
