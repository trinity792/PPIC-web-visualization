...
export function autoMapInlineBindings(chartType, table, previous = {}) {
  ...
  for (const role of roles) {
    if (bindings[role]) continue;
    let pool = columns.filter((c) => acceptsKind(role, c.kind) && !used.has(c.name));
    if (!pool.length) continue;
    // Reserve an explicitly typed Group column for the sectioning role when a
    // regular dimension can fill this role. It remains a dimension and can
    // still fill category/series/etc. when it is the only suitable column.
    if (role !== "group") {
      const regular = pool.filter((column) => !column.isGroup);
      if (regular.length) pool = regular;
    }
    const explicitGroup = role === "group" ? pool.find((column) => column.isGroup) : null;
