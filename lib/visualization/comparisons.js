...
function deriveLabel(comparison, labelMeta) {
  ...
  for (const dimension of labelMeta.dimensionOrder || Object.keys(comparison.dimensions || {})) {
    ...
    if (labelMeta.omitValues?.[dimension]?.includes(value)) {
      omitted.push(String(value));
      continue;
    }
    parts.push(labelForValue(dimension, value, comparison, labelMeta));
  }
  // Aggregate values are dropped because they add nothing next to a real
  // stratum, but a comparison made only of aggregates ("Total"; "All Both
  // Sexes All Ages") must still read as what the reader picked, never as the
  // opaque id.
  return parts.join(" ") || omitted.join(" ") || comparison.id;
}
