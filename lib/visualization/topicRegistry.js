 ...
 * off the module schema. `getTopicHref` routes review-ready modules to v3 and
 * derives the remaining legacy routes from their module ids. Documentation
 * routes are intentionally hardcoded here until document/topic relationships
 * are represented in document metadata.
 *
...
const V3_TOPIC_HREFS = Object.freeze({
...
/** Route into the topic's active chart editor. */
export function getTopicHref(topic) {
  return V3_TOPIC_HREFS[topic.id] || `/${topic.id}`;
}
