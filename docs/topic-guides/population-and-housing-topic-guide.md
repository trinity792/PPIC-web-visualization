...
Content Type: topic guide
pinned: false
description: "As-built topic guide to the Population and Housing pipeline: what it produces, how each phase and frontend boundary works, and a per-script reference with legacy lineage. The first half is for curious non-programmers and the second for programmers."
...
# Population and Housing Topic Guide

The Population and Housing topic turns two families of California Department of Finance (DoF) housing reports into a single, validated, wide CSV of population and housing estimates for every California city, town, county, region, and the state as a whole, from 1991 to the present. This document describes the topic in its refactored form: the shared and Population-and-Housing-specific scripts that replaced the legacy monolith, organized around the six phases the pipeline runs.

> [!info] Who this document is for and how to read it
> This document has two halves. The first half - through "Frontend Surface" - is written for someone who wants to understand what the topic does and how it works without reading code: the data sources, the phases, the calculations, the outputs, and the frontend controls. The second half is a per-script, per-function technical reference for a programmer who wants to know what each piece does and what legacy code it was derived from, faster than reading the source line by line. How the topic evolved from its first-cut refactor to its current state - the reliability, robustness, and efficiency changes and the live-data adaptations - is collected under "From Initial Refactor to Current State."

---

## What the Topic Produces
...
The dataset is "wide" in the sense that all of a place-year's attributes live in one row rather than being melted into a long tidy frame. The frontend visualizations for this topic read this CSV directly, so the pipeline's job is to guarantee that the schema, geography, and year coverage are always complete and internally consistent before the file is overwritten.

## Data Sources

The topic draws on two DoF report families, both discovered by scraping the DoF estimates landing page rather than by hard-coding file URLs that change every year.
...
The E-5 workbook is an Excel file whose second worksheet holds the data. The E-8 workbooks come in three different column layouts across the three decades they cover, which is the single biggest source of complexity in the historical side of the topic.

> [!warning] DoF changes the E-5 format between annual releases - check these first when a run breaks
> The first real end-to-end run against the live 2020-2026 E-5 (2026-07-11) turned up two DoF-side format changes that had nothing to do with the topic's logic, and both are the kind of thing likely to recur each year:
>
...
A third input is the topic's own committed historical baseline. The E-8 history is materialized once into an immutable CSV (`PopHousing_Historical_E8.csv`) by a separate Phase 0 builder, and the main pipeline reads that file back as its pre-2020 history on every run without ever writing it. Earlier revisions pointed the historical path at the pipeline's own output, creating a self-perpetuating history; that has since been decoupled (see "From Initial Refactor to Current State").
...
The topic computes a small number of values rather than passing DoF's numbers through unchanged.
...
The dependency direction is strictly one-way: shared helpers are imported by the `pophousing` files, which are coordinated by the orchestrator. The same California geography reference in `scripts/shared/geography/california_geography.py` is deliberately shared with the Components of Change and Building Permits topics, so the county, region, and metro definitions have a single owner. This mirrors the structure adopted across the other topic migrations; see [[projectSpec]] for the canonical five-phase pipeline model and the audit-status conventions.

## Frontend Surface

The pipeline writes `PopHousing_Current.csv`; the frontend reads it through the shared chart editor and a thin server layer. Two topic-specific controls were added when the provenance and refresh work landed, both driven by flags on the client-safe topic schema (`lib/visualization/moduleSchemas/pophousing.js`).

- **Source provenance filter (multi-select, default all).** Because the output now carries real per-row provenance (`E-5` / `E-8` / `Aggregated`), the schema's `provenanceFilter` flag renders a Source multi-select in the chart sidebar that defaults to every source. The selection is threaded as a comma-separated `sources` query param through the shared `filterRows` helper (which accepts a single value or a list; empty means all) into every `lib/data/pop_housing.js` query and validated in `app/api/pophousing/route.js`. This replaced a stale schema that hard-coded `Source: ["DoF"]`.
- **"Update data" button (background refresh).** The schema's `refreshable` flag renders an "Update data" button that POSTs to `app/api/pophousing/update/route.js`. That route spawns the full pipeline via the project venv (async, detached, guarded by a single-process lock) and returns immediately; the run's outcome is written by the pipeline itself to `logs/pipeline-runs.jsonl` and surfaces on the `/logs` page, so a researcher can pull the latest DoF release without touching the codebase. Operational caveats (auth, the venv-symlink build workaround) are recorded in [[projectSpec]] under "Current-State Notes & Caveats (PopHousing)."

---

## Technical Reference

The rest of this document walks every script the topic comprises, grouped by the phase it serves, with configuration and shared helpers first because every phase depends on them. For each function it gives what the function does, the notable libraries it relies on, any performance or efficiency characteristic worth knowing, and what legacy code it was derived from. The legacy topic was a small number of large files - `pophousing_pipeline.py` (the `main()` monolith), `data_cleaning_utils.py`, `enhanced_forward_fill_helpers.py`, `historical_data_processor.py`, `download_historical_data.py`, `run_original_pipeline.py`, `logging_config.py`, and `pophousing_config.py`. The refactor split those into the many small, single-responsibility topics below.

Every topic follows the same file convention: a docstring header naming its data sources, outputs, usage, and test folder; banner comments separating sections; and one-line docstrings on each function that name the test file covering it.

## Configuration Files

Configuration was one legacy file, `pophousing_config.py`, a flat list of topic-level constants imported directly everywhere. The refactor keeps the raw constants in `lib/pophousing_config.py` but exposes them through four accessor topics that return defensive copies, so no pipeline topic mutates shared state and each phase pulls only the slice of config it needs.
...
Imports `re`, `time`, `pathlib`, `urllib.parse`, `zipfile.BadZipFile`, `pandas`, `bs4.BeautifulSoup`, and the shared HTTP helpers. Defines `E5DiscoveryError` and the topic-level `E5_FILENAME_PATTERN`.
...
This phase is coordinated by `e5_pipeline.py` and draws on five cleaning and calculation topics plus a validator. Together they replace legacy `clean_e5_data()`, `clean_common_e5()`, `add_geographic_level_e5()`, the classification helpers in `enhanced_forward_fill_helpers.py`, and the cleaning parts of `data_cleaning_utils.py`.
...
Imports `pandas` and `get_geography_config`. This topic owns all geographic-level logic.
...
- `assign_geographic_level_with_context(location, county_context, population, housing_row, geography_config)` is the row classifier used across the topic: State, Region, County, Town, ambiguous, or the default City. It receives config as an argument.
...
Derived from legacy `add_geographic_level_e5()`, `add_geographic_level()`, `standardize_san_francisco_classification()`, and the classifier functions in `enhanced_forward_fill_helpers.py`. Keeping one classifier for both historical and modern data is the reason the historical era cleaners can import this topic directly.
...
Imports `pathlib` and `pandas`. `prepare_housing_output(df, output_columns, sort_columns)` validates and stringifies the year, enforces the exact output column order, and sorts, all without mutating the input. It no longer overwrites `Source`: the per-row `E-5` / `E-8` / `Aggregated` provenance from earlier phases flows through to the output (B3), and `Source` is now a required output column. `write_housing_output(df, output_path)` writes to a `.tmp` file and atomically `replace`s it into the final path, cleaning up on failure. Derived from the inline source-setting, column-reordering, sorting, and CSV-writing at the end of legacy `main()`.

`write_housing_output` is no longer wired into the archive-then-write step (see the orchestrator entry below); it survives as a standalone atomic-write utility with its own test coverage. The atomic write alone was not what made "archive then overwrite" safe - Phase 6 used to hand the prior canonical file to `archive_or_delete_files`, which **moved** it, leaving no valid canonical CSV on disk between the move and this write. That was the one real data-loss window documented in `docs/archive/shared-archive-and-save-guide.md` and is now closed by copying instead (see the orchestrator entry below).

### `validation/final_dataset_validator.py`

Imports `pandas` and the shared validators. `validate_final_housing_dataset(df, validation_config)` is the most thorough check in the topic: schema completeness, duplicate output keys, null and invalid and missing geographic levels, null or unknown `Source` values (the new `valid_sources` gate - B3), presence of a California state row, a complete year span with no future years, the San Francisco City-and-County invariant (and never more than twice per year), a Bay Area 2020 population sanity range, and nonnegative and in-range numeric columns. It returns `(is_valid, messages)`. Derived from the final duplicate check, Bay Area sanity check, and schema enforcement that legacy `main()` did inline before writing.

### `orchestrators/pophousing_pipeline.py`

Imports `pandas` and every phase entry point. `main(logger=None)` runs the six phases inside per-phase `try/except` blocks, wrapping any failure in `PipelinePhaseError` via `_raise_phase_error` so a failure is always attributed to a named phase. It owns the cold-start baseline check and freshness log, the E-5 acquisition fallback chain (discover, download, most-recent-cache-or-archive), the merge keys and source priority, the enrichment calls, the Phase 6 missing-level invariant assertion, the final normalization and validation, the archive-then-write, and the returned summary dict.

Phase 6's save step no longer calls `archive_or_delete_files` + `write_housing_output` directly; it calls the shared `archive_and_save` helper (`scripts.shared.archives.dataset_archive`, imported in the orchestrator) with `module_id="pophousing"`. The helper **copies** the prior canonical file into the archive under a topic-prefixed ISO name (`pophousing_PopHousing_2026-08-07.csv`, replacing the old undated `PopHousing_Current.csv` / `_1` counter-suffix scheme) - never moves it - before atomically writing the new data, and no-ops when the data is byte-identical, which this topic never had before. The orchestrator no longer imports `archive_or_delete_files` or `write_housing_output` at all. `archive_or_delete_files` keeps its real job in `scripts/pophousing/archives/e5_retention.py`, moving expired E-5 *source downloads* out of the working directory, which is retention machinery rather than a dataset write path. `write_housing_output` survives in `output/finalize_dataset.py` as a standalone atomic-write utility with its own test coverage, but nothing in the pipeline calls it. The summary dict still reports `paths["current_data_path"]` regardless of whether a write happened, matching the topic's prior always-a-path behavior. See `docs/archive/shared-archive-and-save-guide.md` (archived implementation record). The acquisition guard now also catches read/parse failures on a fresh download and records `e5_discovery_failed`/`e5_download_failed` flags in the summary so a degraded run logs as `recovered` (B2). It emits `log_processing_step` at every phase boundary (Phases 1-6) and contains no transformation logic. Derived from the legacy `main()` monolith and the subprocess-based `run_original_pipeline.py`, both of which are fully replaced; the interactive "continue after failure?" prompt of the legacy runner is gone in favor of structured exceptions.

## From Initial Refactor to Current State

The refactor first split the legacy monolith into the phase topics described above. A later reliability, robustness, and efficiency pass - and the topic's first end-to-end run against live DoF data - then hardened several parts of that initial refactored state into what this guide documents. This section records those changes: what the first-cut refactor left in place, and what it became, so a reader understands why the current code is shaped the way it is.

### Deep history became an immutable, committed artifact

The first-cut refactor pointed the historical path at the pipeline's own output (`HISTORICAL_DATA_PATH = CURRENT_DATA_PATH`), so each run re-derived its pre-2020 history from the previous run's `PopHousing_Current.csv`. That self-perpetuating history carried a slow-corruption risk - a bad pre-2020 row that slipped past the coarse Phase 1 validator would become the next run's "truth" - and it could not cold-start, since a machine with no prior output aborted Phase 1 with a bare `FileNotFoundError`.

Deep history now lives in its own immutable file, `PopHousing_Historical_E8.csv` (`HISTORICAL_BASELINE_PATH`, decoupled from `CURRENT_DATA_PATH`), written *only* by the Phase 0 builder and read-only for the main pipeline. Because the baseline is write-once, the drift channel is gone and the Phase 1 validator is a true acceptance gate on an external artifact rather than a self-check. Cold start raises an actionable "run the Phase 0 builder" error instead of a bare exception. The baseline is kept as a committed, versioned artifact - so fresh installations are reproducible and do not depend on DoF availability - and is regenerated on demand by a manual Phase 0 command, chosen over a scheduled job because the E-8 source changes only about once a decade. A sidecar records the baseline's coverage so Phase 1 can log a network-free freshness warning when a newly configured E-8 era outruns the committed baseline.

### The Phase 0 build was wired up and made runnable

In the first-cut refactor the historical builder (`build_historical_housing_dataset`) and the E-8 downloader had no non-test callers, and `HISTORICAL_FILE_CONFIG` could not be handed to the builder as-is - its per-era entries carried no `path` and no `recover_counties` flag, so a config-driven build would have raised. Both gaps are closed by `historical/build_baseline.py`, the Phase 0 driver: it maps `HISTORICAL_FILE_CONFIG` into the builder's expected shape, runs either a `--from-current` seed or a full workbook rebuild, validates against the shared historical-validation gate, and writes the baseline and its sidecar atomically. Standing the topic up on a clean machine - previously an undocumented, non-working path - is now a covered path, with the runbook [[pophousing-standup]] and a from-clean-state smoke test proving it.

### Structured logging was implemented and given full-phase coverage

The migration plan left the `shared/logging/` topics as `pass`-body stubs. They are now fully implemented: `pipeline_logging.py`, `dataframe_logging.py`, and `run_records.py` provide a file+console logger (with duplicate-handler guards), the `log_processing_step` / `log_dataframe_info` / `log_data_quality_check` helpers (all no-op safely when `logger is None`), and `execute_pipeline_run`, which wraps `main`, appends one JSONL record per run to `pipeline-runs.jsonl`, and is the orchestrator's `__main__` entry point. The initial wiring only logged Phases 1, 3, and 6, leaving acquisition, merge, and enrichment dark; `main` now emits a step log at every phase boundary (Phases 1-6). The acquisition fallback also sets summary flags matching `run_records`' fallback pattern, so a run that fell back to a cached workbook is recorded as `recovered` rather than a clean `success` (see the acquisition-robustness change below).

### The geographic classifier was simplified, sped up, and stopped rebuilding config

Three cleanups landed on the classification path. First, `has_meaningful_housing_data` - a row-level check the vectorized `drop_empty_rows_without_data` had fully subsumed - was removed as dead parity code. Second, `classify_ambiguous_location`'s ten-row "County Total" lookahead was removed: by the time it runs, `resolve_county_total_rows` and `normalize_state_total_rows` have already promoted every real County/State row, so the neighbor scan was redundant in the wired path. (Its rebuild recipe is preserved in the function's description under Phase 3, so a future input that needs it can be re-added quickly.) Third, the geography config is no longer rebuilt per row: the first-cut refactor called `get_geography_config()` inside the per-row classifier - each call re-running `_derive_metro_to_region`'s validation over 26 metros - whereas the config is now built once and threaded through `classify_ambiguous_location`, `assign_missing_geographic_levels`, and `standardize_location_column`.

The classifier's per-row loop itself was the dominant remaining CPU cost, and the one place the first-cut refactor had *not* vectorized. It now assigns the unambiguous cascade (State / Region / County / Town / default) with boolean masks over the whole missing-level subset and reserves per-row work for the small minority of genuinely ambiguous names. A parity test asserts the vectorized result equals a straight per-row application of the classifier.

### E-5 acquisition became genuinely fault-tolerant

Two acquisition weaknesses were fixed. The cache and fallback windows were originally both 60 days, and Phase 1 retention archived any workbook older than 60 days *before* acquisition ran - so during a DoF outage the fallback (looking for a workbook ≤ 60 days old in the download directory) could find nothing, even with a perfectly usable older workbook sitting in the archive. The cache/retention window is now 90 days and the fallback window 400 days, and `get_most_recent_e5_file` searches both the download cache and the archive, so a retention-archived workbook is still reachable. Separately, a corrupt *fresh* download used to abort the run: the Phase 2 guard caught only `HTTPDownloadError`, so a truncated `.xlsx` (a `ValueError` from the reader) escaped the fallback. The guard now also catches read/parse failures and falls back to cache, while a missing `openpyxl` still fails hard - a missing dependency should not silently serve stale data. Both degraded paths set a summary flag so the run is logged as `recovered`.

### The output regained real provenance, and two data-fidelity traps were closed

Phase 6 originally overwrote `Source` to the literal `"DoF"` for every row, discarding the `E-5` / `E-8` / `Aggregated` labels the earlier phases had assigned. The column was dead weight - any QA that trusted it (observed vs. aggregated rows) was silently wrong - and the merge only kept working because `load_historical_housing_data` re-labels the baseline to `E-8` on read. `prepare_housing_output` now preserves the per-row value, `Source` is a required output column, and the final validator gates on the known label set, so a stray value surfaces instead of passing. The chart sidebar exposes the three provenance values as a multi-select (default all); see "Frontend Surface."

Two adjacent traps were closed at the same time. Phase 6 ran a missing-level classification pass keyed on a `County` column that Phase 3 had already dropped, so `row.get("County")` was always `None` and the pass could only ever misclassify; it was removed and replaced with a loud invariant assertion that fails the run if any row somehow reaches Phase 6 without a level. And the vacancy-rate decimal-fraction correction, which rescales DoF vintages that store `0.07` for `7%`, originally flagged any recent rate in the open `(0.01, 1.0)` band - so a genuine sub-1% vacancy (say `0.5%`) would be silently rescaled to `50%`. It now decides per vintage: a year is treated as fraction-encoded only when the median of its rates is below `1.0`, so a lone real sub-1% rate in an otherwise percent-encoded year is left alone.

### Aggregation stopped re-detecting its additive columns

`build_regional_rows` used to re-scan every column to decide which were "safely additive" once per region - nine times per run, plus the state rollup - though that set is identical across regions for a given frame. Detection was split into its own `detect_additive_columns` and is now run once on the deduplicated county frame and reused for every region.

### Live-data adaptations from the first end-to-end run

The baseline and acquisition changes above individually degrade gracefully but compounded on a fresh deployment, which is why the immutable baseline, the Phase 0 driver, and the wider fallback were treated as one "baseline bootstrap" effort backed by the runbook and a smoke test. The topic's first run against the live 2020-2026 E-5 then surfaced two DoF-side format changes, both absorbed and documented in the "DoF changes the E-5 format" callout under "Data Sources": the workbook filename gained an underscore (`E-5_2026_...`, where patterns had expected `E-5-2025_...`), and the per-county subtotal rows are now labelled `Incorporated` / `Unincorporated` instead of `Balance of <County>`. That same run also exposed a latent San Francisco bug - the City/County duplication could expand SF into four rows for a boundary year when an `E-8` City row and an `E-5` County row differed only in source - which was fixed by collapsing SF to one canonical record per year (preferring the more recent source) before emitting its City and County rows.
...
| `clean_e5_data()` | `cleaning/e5_pipeline.py` (orchestration) + the Phase 3 topics |
...
| `historical_data_processor.py` | The seven `historical/` topics |
| `logging_config.py` | `shared/logging/` (`pipeline_logging.py` + `dataframe_logging.py` + `run_records.py`, fully implemented) |
| `pophousing_config.py` | `lib/pophousing_config.py` + the four `config/` accessor topics + `shared/geography/california_geography.py` |

## Current State

Everything described above is implemented and covered by tests, and the pipeline has run end to end against the live 2020-2026 DoF E-5: `PopHousing_Current.csv` holds 19,692 rows spanning 1991-2026, with no duplicate keys and real `E-5` / `E-8` / `Aggregated` provenance. The two frontend controls - the `Source` provenance multi-select and the "Update data" button - are built and verified, and are described under "Frontend Surface." What remains is operational rather than structural: the historical baseline is rebuilt only when DoF publishes a new E-8 decade (through the Phase 0 builder, prompted by the freshness warning), and the "Update data" route should sit behind auth in a shared deployment. Those operational caveats are recorded in [[projectSpec]] under "Current-State Notes & Caveats (PopHousing)."
