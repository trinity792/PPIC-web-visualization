...
pinned: false
description: "As-built topic guide to the Demographic Projections V3 pipeline: a non-technical overview of what it produces and how the five phases run, followed by a per-function programmer reference with legacy lineage. Covers DoF P-3 projections and Census estimates merged into one stratified contract dataset."
...
- **Domain layer** (`scripts/projections/`) - config, acquisition, cleaning, aggregation, merging, validation, and output topics specific to this dataset.
- **Orchestrator** (`scripts/orchestrators/projections_pipeline.py`) - wires the domain topics into the five phases and owns the fallback and offline logic.

The dependency direction is strict: shared -> projections -> orchestrator. Unlike the legacy topic, there is **no Python visualization layer** - the legacy `visualize_line/bar/map()` methods were dropped and replaced by the shared React charting UI. The frontend deliverables (`lib/visualization/moduleSchemas/demographicProjections.js`, `lib/data/demographic_projections.js`, `app/api/projections/route.js`) consume the contract CSV over an HTTP API and are described at the interface level in the second half.
...
- Topic constants: `P3_RAW_COLUMNS`, `CCEST_RAW_COLUMNS` (7 identifiers + 14 race-by-sex fields), the 58 FIPS-ordered county names, the 50 state names, canonical age groups, age-bin edges, canonical race groups and sexes, the P-3 race7 map, the Census race-prefix map, and the Census year-code map.
...
*This topic is new* - the legacy code summed on the fly at chart time; storing marginals is a V3 performance decision to keep the browser from summing millions of rows.
...
- **`archive_and_save`** - no longer implemented here. It is the shared helper at `scripts/shared/archives/dataset_archive.py` (this was the topic the streamed-SHA-256 optimization below was originally written for; it is now shared behavior, not local to Projections), imported directly and called with `module_id="projections"`, producing archive names like `projections_DemographicProjections_2026-08-07.csv` in place of the old `mm-dd-yy` stamp. Behavior is unchanged: serializes the frame to CSV bytes, compares against the existing file by **streamed SHA-256** (no second full-file string in memory); on a match it returns `None` (no write, file left byte- and mtime-identical), otherwise it copies the old file into the archive and writes the new bytes **atomically** (staged `.tmp` + `Path.replace`), so a crash mid-write can never corrupt the contract file that doubles as history. The orchestrator only calls this when a source reported new data (the now-trustworthy change flags), so the full-file serialize + hash no longer runs on every idempotent invocation. See `docs/archive/shared-archive-and-save-guide.md` (archived implementation record).
...
- **`lib/visualization/moduleSchemas/demographicProjections.js`** - the client-safe field catalog for the V3 question editor: temporal `Year`; dimensions `Geographic Level`, `Location`, `Age Group`, `Sex`, `Race/Ethnicity`, and `Source`; plus the `Population` measure and its supported calculations. It adds the topic-specific demographic dimensions that Population and Housing and Components of Change do not have.
...
`config/paths.py` set `historical_data_path == current_data_path`, so the pipeline read its own prior output as its baseline — a slow-corruption channel if a bad write ever became the next run's "truth." Deep history now lives in its own file, `DemographicProjections_Historical.csv` (`historical_data_path`, decoupled from `current_data_path`), read-only to the pipeline. Phase 1 loads both the current output and this seed and unions them with `combine_history_sources`, preferring the current output row-for-row while the seed supplies any deep year the current file may have lost. The seed is optional: on a fresh checkout it is absent and the pipeline **cold-starts** on live data plus whatever current output exists — a property this topic already had (empty-contract bootstrap, no dropped years, strict completeness gate) and which the change deliberately preserves.

### The contract file is written atomically

`archive_and_save` overwrote the output in place with `write_bytes`, a regression against the sibling topics — and because the contract file doubles as history, a process killed partway through writing ~100 MB of CSV left a truncated file that became the next run's baseline. It now stages the new bytes to a sibling `.tmp` file and `os.replace`s it into place (atomic on the same filesystem), with the archive `copy2` still running first: archive old → write tmp → atomic replace. The streamed-hash no-op-on-match optimization is retained as a cheap final guard.
...
Everything described above is implemented and covered by tests: the full unit suite passes (1,066 tests) and `ruff` is clean on the touched topics. The whole pipeline was exercised end-to-end offline against the live P-3 and cc-est files (`PROJECTIONS_OFFLINE=1`): it cleans 745,416 DoF rows and 75,600 Census rows, aggregates and validates to **1,718,208 rows**, and — because change detection now compares like grains — logs `dof_new=False, census_new=False` against the unchanged history and skips the write entirely ("No new data detected; file unchanged"), so the costly full-file serialize + hash no longer runs on an idempotent invocation.
