...
Each item links to the fuller problem statement and plan in "Flagged Issues and Fragilities." Ordered roughly by priority; the three "baseline bootstrap" items (A1, A2, B8) are coupled and best done together.

**Reliability / robustness**
- [ ] **(A1)** Give the historical baseline its own immutable path instead of sharing `PopHousing_Current.csv`, and turn the cold-start `FileNotFoundError` into an actionable "run Phase 0" error. *Decision pending: committed artifact vs. regenerated on demand.*
- [ ] **(A2 + B8)** Ship a Phase 0 driver that maps `HISTORICAL_FILE_CONFIG` into the builder's expected shape (it cannot be passed as-is today), add a freshness check, and write a "stand up from scratch" runbook + smoke test. *Decision pending: manual vs. scheduled.*
- [ ] **(B1)** Resolve the E-5 retention/fallback age collision (all three thresholds are 60 days) so the outage fallback can actually reach an archived workbook. *Decision pending: widen fallback window vs. let the fallback search the archive.*
- [ ] **(B2)** Broaden the Phase 2 guard so a freshly-downloaded *corrupt* E-5 falls back to cache instead of aborting the run.
- [ ] **(B3)** Stop overwriting `Source` to the literal `"DoF"` in Phase 6; preserve real `E-5`/`E-8`/`Aggregated` provenance and make the merge robust to it. *Decision pending: per-row provenance vs. dataset-level tag — potentially a breaking schema change if a consumer reads `Source`.*
- [ ] **(B4)** Remove or fix the Phase 6 missing-level pass, which references a `County` column dropped back in Phase 3; add an assertion if any unlabeled row ever reaches it.
- [ ] **(B5)** Replace the per-row decimal-fraction vacancy-rate heuristic with a per-vintage (distribution-based) or component-verified rule so a genuine sub-1% rate is never rescaled to 50%.

**Correctness housekeeping**
- [ ] **(A5)** Confirm whether the ambiguous-location ten-row lookahead is intended to be live during the missing-level pass; if redundant, delete the dead `housing_df`/`row_index` parameters, otherwise thread the frame through.
- [ ] **(A4)** Decide whether `has_meaningful_housing_data` should be removed or explicitly annotated as unused parity code.

**Efficiency**
- [ ] **(B6 + A6)** Vectorize the geographic-classification loop (reserve per-row work for true ambiguities) and thread the geography config through instead of rebuilding it per row — do these together.
- [ ] **(B7)** Detect the additive-column set once per frame and reuse it across regions and the state rollup.

**Documentation**
- [x] Corrected the stale "unimplemented stubs" language in the "Shared Helpers → `shared/logging/`" body text (logging is done — see A3). Still open: fill the Phase 2/4/5 structured-logging coverage gaps.

**Resolved since the original pass**
- [x] **(A3)** ~~Implement the `shared/logging/` interface~~ — done: `pipeline_logging.py`, `dataframe_logging.py`, and `run_records.py` are fully implemented and wired through the orchestrator via `execute_pipeline_run`.
