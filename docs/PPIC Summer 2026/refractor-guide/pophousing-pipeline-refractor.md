...
Last Updated: 07/12/2026 - 12:15 AM
Status: Finalized
...
> This document has two halves. The first half - through "Frontend Surface" - is written for someone who wants to understand what the module does and how it works without reading code: the data sources, the phases, the calculations, the outputs, and the frontend controls. The second half is a per-script, per-function technical reference for a programmer who wants to know what each piece does and what legacy code it was derived from, faster than reading the source line by line. How the module evolved from its first-cut refactor to its current state - the reliability, robustness, and efficiency changes and the live-data adaptations - is collected under "From Initial Refactor to Current State."
...
The pipeline writes `PopHousing_Current.csv`; the frontend reads it through the shared chart editor and a thin server layer. Two module-specific controls were added when the provenance and refresh work landed, both driven by flags on the client-safe module schema (`lib/visualization/moduleSchemas/pophousing.js`).
...
Phase 6's save step no longer calls `archive_or_delete_files` + `write_housing_output` directly; it calls the shared `archive_and_save` helper (`scripts.shared.archives.dataset_archive`, imported in the orchestrator) with `module_id="pophousing"`. The helper **copies** the prior canonical file into the archive under a module-prefixed ISO name (`pophousing_PopHousing_2026-08-07.csv`, replacing the old undated `PopHousing_Current.csv` / `_1` counter-suffix scheme) - never moves it - before atomically writing the new data, and no-ops when the data is byte-identical, which this module never had before. The orchestrator no longer imports `archive_or_delete_files` or `write_housing_output` at all. `archive_or_delete_files` keeps its real job in `scripts/pophousing/archives/e5_retention.py`, moving expired E-5 *source downloads* out of the working directory, which is retention machinery rather than a dataset write path. `write_housing_output` survives in `output/finalize_dataset.py` as a standalone atomic-write utility with its own test coverage, but nothing in the pipeline calls it. The summary dict still reports `paths["current_data_path"]` regardless of whether a write happened, matching the module's prior always-a-path behavior. See [[shared-archive-and-save-plan]]. The acquisition guard now also catches read/parse failures on a fresh download and records `e5_discovery_failed`/`e5_download_failed` flags in the summary so a degraded run logs as `recovered` (B2). It emits `log_processing_step` at every phase boundary (Phases 1-6) and contains no transformation logic. Derived from the legacy `main()` monolith and the subprocess-based `run_original_pipeline.py`, both of which are fully replaced; the interactive "continue after failure?" prompt of the legacy runner is gone in favor of structured exceptions.

## From Initial Refactor to Current State

The refactor first split the legacy monolith into the phase modules described above. A later reliability, robustness, and efficiency pass - and the module's first end-to-end run against live DoF data - then hardened several parts of that initial refactored state into what this guide documents. This section records those changes: what the first-cut refactor left in place, and what it became, so a reader understands why the current code is shaped the way it is.
