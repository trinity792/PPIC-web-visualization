...
Last Updated: 07/21/2026 - 02:01 PM
Status: Updating
...
> NF-1 now ships three composable additions in the standalone Visualization Tool (`byod` schema): a first-class **Group** column type, category sectioning with labeled blocks and adjustable spacing, and `GraphTabs` filtering by any dimension-like inline column. The implementation is covered by the full Vitest suite (488 tests), full-repository eslint, and the production build.
...
1. The data grid exposes `group` as a manual semantic type, while it remains a normal `dimension` for role compatibility.
...
`group` is implemented as a **column type** whose values coerce exactly like `text` (arbitrary trimmed strings, never malformed) and whose field **kind remains `dimension`**. The field descriptor adds `isGroup: true`, which gives auto-mapping and the Group-role picker a preference without adding a new field kind. It is a manual designation only: `inferColumnType` continues to return only `text`, `number`, or `date`, so imported tables begin with zero `group` columns.

As-built changes:

| File | Implemented change |
| --- | --- |
| [`lib/tabular/columnTypes.js`](../../../lib/tabular/columnTypes.js) | Defines `COLUMN_TYPES.GROUP`; `coerceColumn` sends it through the trimmed-string path, while inference remains manual-only. |
...
**Auto-map behavior:** when a chart has the sectioning `group` role and the table has a `group`-typed column, `autoMapInlineBindings` binds it before previous bindings and name synonyms. On a fresh import, ordinary text columns named exactly `Group` or `Category` get that same priority whenever every required role remains fillable from the other columns; this prevents sectioning from stealing the only available category dimension. Ordinary `section`, `block`, and demographic-name synonyms remain the fallback when no stronger group signal exists.
...
- Grouped row axes hide their vertical spine, keeping the rule out of the group-header margin.
...
- The full Vitest run is green: **50 files, 493 tests**. Full-repository eslint is clean except for existing configuration notices. The production build compiles with the existing broad NFT trace warning from `next.config.js` / `app/api/pophousing/update/route.js`.
...
All three live under `filters` (like `subset`/`topN`), so they flow into the preview `requestKey` in [`PreviewContext.js`](../../../components/chart-builder/wizard/PreviewContext.js#L80) and re-render on change with no extra fetch plumbing.

**Where the filter is applied.** `applyTabFilter(table, filters)` runs at the top of `buildShapes`, before query-shape dispatch. Line, category, two-period, pairs, matrix, and geo builders therefore share the same exact row filter. A null or missing column is a no-op; a stale non-null value produces zero rows and the preview's existing empty state.

**The tab options.** `tabValues` returns distinct, trimmed, non-blank values in first-seen order and overlays `tabOrder` when present. The editor allows `text`, `date`, and `group` columns and ranks `group`-typed columns first. The preview feeds these values directly to `GraphTabs`; a click dispatches `SET_FILTER` with the owning chart slot id, so tabs work correctly in multi-chart workspaces.

As-built files:

| File | Implemented change |
| --- | --- |
| [`components/chart-builder/chartConfigStore.js`](../../../components/chart-builder/chartConfigStore.js) | Defaults and synchronizes all three tab fields, resets values on column/data changes, persists custom order, and supports chart-targeted tab actions. |
...
| [`components/chart-builder/EncodingSection.js`](../../../components/chart-builder/EncodingSection.js) | Renders the byod-only **Tab by column** select plus the advanced drag/keyboard tab-order editor. |
| [`components/chart-builder/wizard/PreviewPane.js`](../../../components/chart-builder/wizard/PreviewPane.js) | Renders the existing `GraphTabs` component above each chart slot without a visible source-column label, preserves that context in its accessible name, and targets clicks to that slot's chart id. No duplicate wrapper component was needed. |
| [`components/chart-builder/wizard/PreviewContext.js`](../../../components/chart-builder/wizard/PreviewContext.js) | Required no shape change because `config.filters` already participates in `requestKey`. |
...
2. **Iframe interactivity.** Embedded mode still mounts the live config and preview providers. Clicking a pill updates `tabValue`, changes `requestKey`, re-runs inline shaping, and does not enable the Plotly editing modebar.
...
- Shape tests verify that tab filtering runs before dispatch and composes with category grouping.
...
2. **Column eligibility:** any inline `text`, `date`, or `group` column is eligible; explicit group columns rank first.
