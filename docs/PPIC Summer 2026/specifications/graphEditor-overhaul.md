...
Last Updated: 07/21/2026 - 03:20 PM
Status: Updating
...
[`lib/constants.js`](../../../lib/constants.js) gained the **official style-guide colors** (`officialOrange/Navy/Lime/Blue/Violet/Seafoam/Gray/Red/Green/DarkGray` + a maroon-red scale) and [`palettes.js`](../../../lib/visualization/palettes.js) exposes them as `ppic-official-two-group` plus one selectable categorical option for every group count from 3 through 10. The editor variants make the requested ordering adjustment: Lime is absent from 3–4 groups and appears as the final swatch in every 5–10 group palette. The editor palette registry also exposes all eight named families from the UI Kit's Color foundation — Orange, Blue, Teal, Navy Blue, Steel Blue, Complement Green, Burnt Orange, and Neutral — as token-only categorical cycles; selecting one applies it immediately through the existing `SET_PALETTE` path. `toPlotly` added **typography** (`chartFontFamily` — Source Sans 3 / Arial; per-element `titleFontSize`/`subtitleFontSize`/`axisFontSize`/`dataLabelFontSize`/`legendFontSize`) and **legend-label wrapping** (`wrapLegendLabel`/`escapeLegendText`, gated by `legendWrap`/`legendWrapChars`), surfaced as "Chart typography" / "Legend text size" controls in the Appearance section and `chartTypography`/`legendTypography` tier entries. Legend marker size follows each trace's `marker.size`; chart CSS does not override the SVG transform Plotly uses to position colored swatches.
...
## NF-1 — A "Group" column type + group-by across applicable charts

> [!success] Status: BUILT AND AUTOMATED-VERIFIED - 2026-07-21
> NF-1 now ships three composable additions in both the standalone Visualization Tool (`byod` schema) and the module visualization editors: a first-class **Group** column type for imported data, category sectioning with labeled blocks and adjustable spacing, and `GraphTabs` filtering by an eligible dimension. The implementation is covered by the full Vitest suite (505 tests), changed-file eslint, and the production build.

> [!example] What "group" means (worked example — a grouped range plot)
> The reference is a Datawrapper **range/dot plot**, "Gender pay gap": one row per `Label` (e.g. *Graduate Degree*, *Dentists*), two dots per row (*Women*/*Men* = the range endpoints), a `+%` gap annotation, and — the point of this feature — the rows **sectioned under bold group headers** (*By education level*, *By occupation*) with visible whitespace between the two sections. The `Group` column is what drives that sectioning. It is **not** a series split (clustered bars / multiple lines); series-splitting stays on the pre-existing `series`/`color` roles and is orthogonal.
>
> Source data (the columns map as: `Label`→category, `Group`→section, `Women`→start, `Men`→end, `Total`→optional center point):
>
> | Label | Group | Total | Men | Women |
> | --- | --- | --- | --- | --- |
> | Less than Bachelor's Degree | By education level | 41330 | 46680 | 35680 |
> | Bachelor's Degree | By education level | 66580 | 77220 | 57340 |
> | Bachelor's Degree or Higher | By education level | 74010 | 86370 | 63860 |
> | Graduate Degree | By education level | 87200 | 102500 | 75120 |
> | Chief executives | By occupation | 151000 | 160400 | 121500 |
> | Dentists | By occupation | 152100 | 170100 | 140700 |
> | Lawyers | By occupation | 131700 | 150500 | 116100 |
> | Software developers | By occupation | 112400 | 116600 | 101600 |
> | Registered nurses | By occupation | 70380 | 75050 | 69780 |
> | Retail salespersons | By occupation | 39650 | 45110 | 31530 |
> | Postsecondary teachers | By occupation | 71410 | 76740 | 66820 |
> | Preschool and kindergarten teachers | By occupation | 31790 | 32410 | 31770 |
> | Elementary and middle school teachers | By occupation | 53030 | 57380 | 52220 |
> | Secondary school teachers | By occupation | 57400 | 60330 | 56190 |

### As-built architecture

NF-1 extends the existing field, shape, config, and preview paths rather than introducing a parallel chart system:

- **Column types** are `text | number | date`, defined in [`lib/tabular/columnTypes.js`](../../../lib/tabular/columnTypes.js) (`COLUMN_TYPES`) and offered in the data-editor dropdown ([`InputTableEditor.js`](../../../components/chart-builder/InputTableEditor.js), `COLUMN_TYPE_OPTIONS`). Each maps to a field **kind** through [`inlineColumnKind`](../../../lib/visualization/inlineMapping.js): `text → dimension`, `number → measure`, `date → temporal`.
- **Charts declare a few dimension roles already.** In [`lib/visualization/chartRegistry.js`](../../../lib/visualization/chartRegistry.js), `line` has `series`/`color`, `bar` has `group`/`color`, scatter/bubble have `color` — all `DIMENSION`. The encoding UI ([`EncodingSection.js`](../../../components/chart-builder/EncodingSection.js)) already renders a dropdown per role and lists every dimension column, and `roleLabel` already prints **"Group"** for the `group` role. But today these roles all mean **series-splitting** (one line/colored trace per value) — *not* the category **sectioning** this feature is about.
- **Series-splitting for line charts already works** and is a **separate** capability we keep as-is: [`buildLineShape`](../../../lib/tabular/toSeries.js) already draws one line per distinct value of `bindings.series || bindings.color`. The Group column does not touch this.

**The shipped additions are:**

1. The data grid exposes `group` as a semantic type, while it remains a normal `dimension` for role compatibility. Imports select it automatically for exact `Group` or `Category` headers; every other header remains user-selectable.
2. Category, two-period, and dot-plot matrix shapes carry the selected group without changing ungrouped module-compatible responses.
3. The Plotly adapter creates stable category blocks, group-header annotations, and numeric spacer positions controlled by `appearance.groupGap`.
4. The editor can tab-filter every inline query shape before shaping, with the shared `GraphTabs` control rendered inside the preview and embeds.
5. Module editors expose the same Group and Tab controls. Module stratification groups are loaded through their existing API filters and merged as grouped chart records; stable dimensions (for example RHNA Region) are joined from the module's full-width table. Module tabs either drive the matching API filter directly or narrow the plotted location set, preserving transforms, map joins, and line-layer loaders.

---

### Part A — the "Group" column type

`group` is implemented as a **column type** whose values coerce exactly like `text` (arbitrary trimmed strings, never malformed) and whose field **kind remains `dimension`**. The field descriptor adds `isGroup: true`, which gives auto-mapping and the Group-role picker a preference without adding a new field kind. Value-only inference continues to return `text`, `number`, or `date`; header-aware import inference returns `group` when the trimmed, case-insensitive header is exactly `Group` or `Category`. All other columns require manual selection if they should carry group semantics.

As-built changes:

| File | Implemented change |
| --- | --- |
| [`lib/tabular/columnTypes.js`](../../../lib/tabular/columnTypes.js) | Defines `COLUMN_TYPES.GROUP`; `coerceColumn` sends it through the trimmed-string path, while header-aware inference selects it only for exact `Group`/`Category` names. |
| [`components/chart-builder/InputTableEditor.js`](../../../components/chart-builder/InputTableEditor.js) | Includes `group` in the existing type `Select`; the type-agnostic setter required no special case. |
| [`lib/tabular/tableChecker.js`](../../../lib/tabular/tableChecker.js) | Grades both `text` and `group` as the existing text grade, including numeric-looking group labels. |
| [`lib/visualization/inlineMapping.js`](../../../lib/visualization/inlineMapping.js) | Maps `group` to `DIMENSION`, publishes `isGroup: true`, describes it as a group in user messages, reserves it for the sectioning role, and ranks it first in that role's picker. |
| [`lib/tabular/toSeries.js`](../../../lib/tabular/toSeries.js) | Counts `group` in the dimension total used by `supportedShapes`, including bar and geography eligibility. |

**Auto-map behavior:** a fresh import types exact `Group` or `Category` headers as `group`. When a chart has the sectioning `group` role, `autoMapInlineBindings` gives that typed column sectioning priority only when every required role remains fillable from the other columns. The exact-name fallback remains for older saved inline tables whose column is still typed `text`. Across chart switches, valid required bindings are preserved and missing required roles are filled before optional bindings from the previous chart are restored; this prevents an optional Range `group` from consuming Dot plot's only available `x` dimension. Ordinary `section`, `block`, and demographic-name synonyms remain the fallback when no stronger group signal exists.

---

### Part B — category-axis sectioning by the Group column

"Group" = **partition a chart's category axis into contiguous, labeled blocks**, one block per distinct value of the `group` column. "Applicable" charts are therefore the ones that **lay categories out along an axis** (a row list or a category axis) — not the continuous, geographic, or single-whole charts.

#### How the visuals should work

Using the *Gender pay gap* example above (a Range/dot plot with `Group` = *By education level* / *By occupation*):

1. **Blocks in group order.** The rows are reordered so all categories of one group are contiguous, blocks following the group column's **first-seen order** (*By education level*, then *By occupation*). Within a block, the chart's existing sort still applies — sorting happens **inside** each block, never across the whole axis (so *Dentists* sorts among occupations, not against *Graduate Degree*).
2. **A bold group header per block.** Each block is labelled with its group value (*By education level*) as a section heading on the category axis — rendered as an axis annotation, left-aligned above/beside the block, visually heavier than the category labels. Grouped row charts render the y-axis as one segment per block, preserving the axis while leaving a break around each header and the inter-group whitespace.
3. **Whitespace between blocks.** Blocks are separated by extra empty space so the sections read as distinct — this is the gap the *Padding control* (below) adjusts.
4. **Everything per-row is unchanged.** Endpoints (Women/Men dots), the connecting bar, the `+%` gap label, colours, value labels — all render exactly as they would ungrouped. Sectioning only reorders rows and inserts headers/whitespace; it never changes a row's marks.
5. **No group column bound → today's behavior.** With `group` unset the chart is a single flat block, identical to now.

#### Padding control (user-adjustable group spacing)

The implemented appearance setting is **`appearance.groupGap`**, labeled **"Space between groups"**. It is a moderate-tier `Slider` from 0 to 3 category units in 0.25 steps, defaults to `0.75`, and appears only when the active sectioning chart has a bound `group` role. It persists through the normal `appearance` container in saved views, workspace exports, and embeds.

#### Applicable charts

| Chart family (query shape) | Sections by `group`? | Notes / work |
| --- | --- | --- |
| `dumbbell` (Range, twoPeriod) | **Yes — primary** | The worked example. `buildTwoPeriodShape` carries a `group` per category; `toPlotly` sections the row axis. |
| `dotPlot` (matrix) | **Yes** | Row-list dot plot — same row-axis sectioning. |
| `forest` (twoPeriod) | **Yes** | Studies grouped by subgroup — same as dumbbell. |
| `slope` (twoPeriod) | **Yes, renderer-adapted** | The existing slopegraph has numeric value axes rather than a category axis. Its categories are therefore grouped into bold Plotly legend-group titles with `tracegroupgap`; the underlying endpoint marks remain unchanged. |
| `bar` / `divergingBar` (category) | **Yes** | Bars grouped into blocks with headers + gaps along the category axis. `divergingBar` **gains a `group` role** (resolved, open question 2). |
| `line` (line) | **No** | Continuous time axis — no categories to section. (Series-splitting via `series`/`color` is the separate, existing capability.) |
| `scatter` / `bubble` (pairs) | **No** | Continuous x/y — sectioning doesn't apply. |
| `pie` (category) | **No** | A single whole; blocks have no meaning. |
| `heatmap` (matrix) | **No** for v1 | Two-dimension grid; sectioning rows is possible but deferred. |
| `choroplethMap` / `symbolMap` (geo) | **No** | Geography is the layout. |

#### `group` role vs. series-splitting

`group` is redefined/standardized as the **sectioning** role. This changes the meaning of `bar`'s existing `group` role from "series split" to "section the category axis"; series-splitting (clustered/stacked bars, colored traces) stays on `color`/`series` and is unaffected. So a bar chart can *both* section by `group` **and** split by `color` — the two are orthogonal. (Open question 3's clustered/stacked default therefore refers to the `color`/`series` dimension, not to `group`.)

[`chartRegistry.js`](../../../lib/visualization/chartRegistry.js) now declares the optional `[DIMENSION]` `group` role and the moderate-tier `groupGap` default on `bar`, `divergingBar`, `dumbbell`, `dotPlot`, `forest`, and `slope`. `roleLabel` continues to provide the shared **Group** label.

#### Data-shape + render work

- **Shape builders ([`toSeries.js`](../../../lib/tabular/toSeries.js)):** category and two-period records receive an optional `group` field only when the role is bound. Bar `color` is carried separately, so series splitting remains orthogonal. Dot plots use the existing matrix envelope plus a parallel `matrix.groups` array aligned to `matrix.y`; this is the only matrix-compatible representation because matrix rows are not record objects. No group binding means the existing module-compatible response is unchanged byte for byte.
- **Ranking ([`chartData.js`](../../../components/chart-builder/chartData.js)):** Top/Bottom selection remains the existing global selection. After selection, records are restored to first-seen group order while retaining the requested value sort inside each surviving block. Matrix ranking reorders `y`, `z`, and `groups` together.
- **Renderer ([`toPlotly.js`](../../../lib/visualization/toPlotly.js)):** the exported pure helper `groupedCategorySections` computes group order, in-block category order, numeric spacer positions, and header positions. `withGroupedCategoryAxis` applies tick labels, range, and one bold annotation per block. Bars, diverging bars, Range, forest, and dot plots share this path; slopegraphs use the grouped-legend adaptation described above.

> [!important] Inline/module compatibility preserved
> Ungrouped category, two-period, and matrix responses remain byte-identical to their module-path builders. Grouped category/two-period data adds only the optional record key; grouped dot plots add only the row-aligned `matrix.groups` metadata required by the matrix shape.

**Validation.** `validateInlineBindings` automatically accepts and existence-checks `group` because the role is now declared by each applicable registry descriptor. Existing chart complexity warnings continue to cover excessive category counts; NF-1 adds no blocking group-count rule.

---

### Completed implementation sequence

1. Part A added the `group` type end to end, followed by coercion, grading, field-hint, supported-shape, and auto-map tests.
2. The shape spike confirmed optional record metadata for category/two-period data and required parallel row metadata for the matrix path.
3. Range established the shared numeric-position sectioning helper and group-header annotations against the worked gender-pay-gap shape.
4. Bar, diverging bar, forest, dot plot, and slope then reused the shared block ordering or the renderer-appropriate legend variant.
5. The moderate-tier `groupGap` slider and persistence path completed Part B.

### Post-build polish (2026-07-21)

- Inline Range/slope endpoints now derive their display names from the distinct bound columns, so the worked example's legend reads **Women**, **Men**, and **Total** (with their marker colors) rather than exposing internal period indices `0` and `1`. Module-backed two-period charts continue to use actual year labels.
- Grouped row axes replace the continuous vertical spine with per-block segments, keeping the y-axis visible without running a rule through group headers.
- The graph editor includes all eight UI Kit color-family palettes, each applied immediately when selected.
- Advisory chart-recommendation cards are no longer rendered; actionable configuration errors remain.
- Tab rows keep an accessible name derived from their source column but omit the redundant visible column label.
- Exact `Group` and `Category` headers receive safe automatic group-role preference as described above.
- Wide pay-gap-style data now defaults to Range and maps `Women` → `start`, `Men` → `end`, and `Total` → `point`. Switching repeatedly between Range and Dot plot keeps every Dot plot required role valid; stale optional group state can no longer displace `x`.
- Range legend swatches retain Plotly's per-marker SVG translation. The former CSS `transform: scale(...)` override was removed because it replaced that translation and moved the colored marker paths out of their legend slots.
- Appearance includes official PPIC categorical palette options for 3–10 groups. Lime is excluded from the 3/4-group choices and is always the final color from the 5-group choice onward.
- Paste, delimited-file, and spreadsheet imports automatically set the data-editor column selector to **Group** when a header is exactly `Group` or `Category` (trimmed and case-insensitive); value-only inference and all other header names retain their existing behavior.
- Module visualization editors now expose eligible catalog dimensions in **Tab by column** and non-high-cardinality dimensions in the Group role. Static schema values appear immediately; data-derived values such as RHNA Region are synchronized after the source table loads.
- Module tabs reuse the existing module filters where possible (Age Group, Sex, Race/Ethnicity, Tenure, Income Level, and Source) and otherwise constrain the existing chart request by matching locations. Grouping by a module stratification loads each selected stratum through the established chart API and merges the results with aligned group metadata.
- Runtime module tab options are treated as computed state, while `tabColumn`, `tabValue`, and `tabOrder` remain the serialized source of truth for saved views and embeds. Group columns are retained in category, Range, and matrix data exports.

### Verification

- `columnTypes`, `tableChecker`, `inlineMapping`, `toSeries`, `chartData`, `chartConfigStore`, `savedViews`, `PreviewPane`, `chartRegistry`, and `toPlotly` have NF-1 coverage.
- The direct renderer test verifies category ordering, exact spacer positions, and one header per block; family tests cover bar/color composition, Range marks, dot plot, forest, and slope legend groups.
- The full Vitest run is green: **51 files, 505 tests**. Every changed JS file is eslint-clean; the source-wide pass is otherwise clean but still reports the pre-existing unused `within` import in `tests/js/components/logs/ChangelogBrowser.test.js`, plus the existing React-version configuration notice. The production build compiles with the existing broad NFT trace warning from `next.config.js` / `app/api/pophousing/update/route.js`.
- A browser click-through remains recommended before release-signoff because automated tests do not assess the visual weight of headers at every responsive width.

### Resolved Part A/B decisions

1. **Kind:** `group` is a `dimension` plus `isGroup: true`, not a new field kind.
2. **Diverging bar:** it has the same optional `group` role and spacing control as bar.
3. **Bar series default:** `stackMode: "none"` remains clustered. `color` owns the split; `group` owns sections. Existing stacked/percent controls remain available.
4. **Code editor:** R/Stata grammar support remains deferred with the broader code-editor backlog. GUI configs and JSON saved views support NF-1 fully.

---

### Part C — GraphTabs that *filter* the chart by a chosen column

> [!note] Filtering ≠ grouping
> Part B **sections** one chart's category axis into labeled blocks shown *at once* (all groups visible, with headers + spacing). Part C **filters**: the user picks a column, one pill-tab appears per distinct value, and clicking a tab shows **only the rows matching that value** — one slice at a time. They are independent and composable (e.g. tab by `Region`, section by `Group`).

**Design language - the RHNA tabs are reused verbatim.** The "Median on-track score by region" card's income-level toggle is [`components/charts/GraphTabs.js`](../../../components/charts/GraphTabs.js), extracted from [`RegionalOnTrackBars.js`](../../../components/landing/RegionalOnTrackBars.js). Part C renders through that same component (pill buttons: `rounded-full px-3 py-1 text-xs font-medium`, active `bg-ppic-blue-300 text-white`, inactive `bg-muted … hover:bg-muted/70`). The graph preview omits a redundant visible column label while supplying `Filter chart by <column>` as the button group's accessible name, so `GraphTabs` remains the single owner of this pattern per [[graph-tabs-facet-selector]].

**As-built state model.** Three filter fields serialize into saved views and embed specs:

- `config.filters.tabColumn` — the column name to tab by (`null` = no tabs; the default).
- `config.filters.tabValue` — the currently active tab value. Defaults to the first distinct value of `tabColumn`; re-derived (and reset) whenever `tabColumn` changes or the data changes.
- `config.filters.tabOrder` - the persisted order from the advanced drag/keyboard reorder control. It defaults to first-seen order, removes values that disappear, and appends new values in first-seen order.

All three live under `filters` (like `subset`/`topN`), so they flow into the preview `requestKey` in [`PreviewContext.js`](../../../components/chart-builder/wizard/PreviewContext.js#L80) and re-render on change. Module-loaded option lists live in runtime-only `tabOptions`; they are stripped from serialized specs and reconstructed from schema values or the module source table.

**Where the filter is applied.** For imported data, `applyTabFilter(table, filters)` runs at the top of `buildShapes`, before query-shape dispatch. Line, category, two-period, pairs, matrix, and geo builders therefore share the same exact row filter. For module data, catalog-backed stratifications and Source tabs update the existing API parameters; other eligible dimensions are resolved against `view=table` and passed to the chart request as its matching location set. A null or missing column is a no-op; an empty slice produces the preview's existing empty state.

**The tab options.** `tabValues` returns distinct, trimmed, non-blank inline values in first-seen order and overlays `tabOrder` when present. Imported data allows `text`, `date`, and `group` columns and ranks `group`-typed columns first. Module editors offer non-high-cardinality catalog dimensions; declared schema values are immediate and undeclared values are derived from the current module table. The preview feeds either source directly to `GraphTabs`; a click dispatches `SET_FILTER` with the owning chart slot id, so tabs work correctly in multi-chart workspaces.

As-built files:

| File | Implemented change |
| --- | --- |
| [`components/chart-builder/chartConfigStore.js`](../../../components/chart-builder/chartConfigStore.js) | Defaults and synchronizes all three tab fields for inline and module sources, stores runtime module options as computed state, persists custom order, and supports chart-targeted tab actions. |
| [`lib/tabular/toSeries.js`](../../../lib/tabular/toSeries.js) | Exports `tabValues` and `applyTabFilter`; every inline shape is filtered before dispatch. |
| [`components/chart-builder/EncodingSection.js`](../../../components/chart-builder/EncodingSection.js) | Renders **Tab by column** for imported and module dimensions plus the advanced drag/keyboard tab-order editor; the Group role uses the same module catalog while excluding high-cardinality location fields. |
| [`components/chart-builder/chartData.js`](../../../components/chart-builder/chartData.js) | Resolves module tab options, translates module filter dimensions to their existing query parameters, narrows stable-dimension tabs by location, joins stable group metadata, and merges grouped stratification requests without replacing chart-specific loaders. |
| [`components/chart-builder/wizard/PreviewPane.js`](../../../components/chart-builder/wizard/PreviewPane.js) | Renders the existing `GraphTabs` component above each chart slot from inline or module-loaded options, preserves the source-column context in its accessible name, and targets clicks to that slot's chart id. No duplicate wrapper component was needed. |
| [`components/chart-builder/wizard/PreviewContext.js`](../../../components/chart-builder/wizard/PreviewContext.js) | Feeds module tab metadata back into computed config state and suppresses a redundant group header when the active tab and group binding use the same dimension. |
| [`lib/export/exportTable.js`](../../../lib/export/exportTable.js) | Retains group columns in category, Range, and matrix exports. |

---

### Part C2 — tabs must work in embedded views

Embeds render the **preview only**: `/visualization-tool?embed=1` sets `embedded` on [`VisualizationWizard`](../../../components/chart-builder/wizard/VisualizationWizard.js#L104), which mounts `<PreviewPane embedded />` with no wizard chrome (no sidebar, no EditStep). So the "Tab by column" *picker* stays author-side, but the **tab bar itself must live inside the render surface** — i.e. rendered by `PreviewPane`/the chart slot, not by `EditStep` — or it vanishes in the iframe.

As-built embed behavior:

1. **Preview-owned rendering.** `PreviewPane` renders `GraphTabs` above the chart content inside every `ChartSlot`. The author-only picker remains in `EncodingSection`, while the resulting pills are part of both the wizard and iframe render surface.
2. **Iframe interactivity.** Embedded mode still mounts the live config and preview providers. Clicking a pill updates `tabValue`, changes `requestKey`, and re-runs inline shaping or the module query path without enabling the Plotly editing modebar.
3. **Serialization.** Single-chart views and multi-chart workspace embeds serialize the complete `filters` object, including `tabColumn`, `tabValue`, and `tabOrder`. Embeds open on the saved value.
4. **Embed CSS.** Existing `.chart-embed-mode` selectors hide only the top-level navigation and fixed chrome. They do not target the tab row; embedded pills use the same compact preview padding.

**Verification (C + C2):**

- Unit tests cover option extraction, blank exclusion, custom ordering, filtering/no-op/empty behavior, store resets after data changes, and saved-view serialization.
- Shape tests verify that inline tab filtering runs before dispatch and composes with category grouping. Module loader tests cover stable-dimension location filtering, direct stratification parameters, and multi-stratum grouped responses.
- `PreviewPane` tests render embedded mode, confirm both pills are present, and verify a click dispatches `SET_FILTER` to the correct chart id.
- The shared `GraphTabs` component's existing controlled/uncontrolled and accessibility tests remain green.

**Resolved Part C decisions and future merge option:**

1. **Tab order:** first-seen is the default. Advanced mode exposes the same drag-and-keyboard ordering pattern used for line/category values, persisted as `tabOrder`.
2. **Column eligibility:** any inline `text`, `date`, or `group` column is eligible; explicit group columns rank first. Module tabs use non-high-cardinality dimensions, while module Group roles also omit Source because module sources have their own constrained selector.
3. **Facet interaction:** tabs and facets remain distinct. Facets mean all slices rendered simultaneously as small multiples; tabs mean one interactive client-side slice. They can therefore coexist as separate choices without changing each other's config.

> [!note] Detailed future merge option
> If user research shows that two authoring controls are confusing, a later spec version could replace them with one **Slice by** field plus a presentation choice: **Tabs (one at a time)** or **Facets (all at once)**. The shared state would own the column, value order, and blank-value policy; the presentation mode would decide whether shaping filters to one value or emits every value as a facet. Migration would map `filters.tabColumn` to `slice.column` + `slice.presentation = "tabs"` and `bindings.facet` to `slice.column` + `slice.presentation = "facets"`. If both are present in an older chart, migration must preserve both rather than guess: keep tabs as the outer interactive slice and facet the already-filtered rows inside it, matching today's composable behavior. This remains deferred because the current renderer does not yet implement generic small-multiple facets across chart families.
