...
> With Building Permits migrated, **all five V1 legacy datasets now run on the V3 architecture.** No original notebook dataset remains un-migrated; further work is enhancement (deeper history, new views, the graph-editor overhaul) rather than net-new module migration. The **RHNA Progress Report** module (added 2026-07-16) is the first exception to that framing — the first **net-new** module built on the V3 architecture with no legacy predecessor, currently built and test-first-green offline with a verified live CKAN run pending.
...
A plain-language summary of what sits inside each contract - what the data measure, how far they reach in time and geography, and the questions they are built to answer.

- **Population & Housing (PopHousing)** - Annual counts of people and housing units for California from 1991 through 2026, covering the state, the 9 PPIC regions, all 58 counties, and roughly 485 cities and towns, with population split into household and group-quarters residents and housing split by structure type (single family, multi-family, mobile homes) alongside occupancy, vacancy rate, and persons per household.
  - It answers "how much has this place grown, and has its housing stock kept up," and is the only module with city-level detail, so it carries any question that needs to go below the county.
- **Components of Change** - Annual births, deaths, natural increase, and net migration (split into domestic and foreign) from 1991 through 2025, with matching crude rates per 1,000 residents, for California's state, regions, and counties from DoF plus all 50 states and DC from the Census.
  - It explains *why* a place grew or shrank rather than just by how much, and its state-level Census rows let California be benchmarked against the rest of the country.
- **Age, Sex & Race Projections (Demographic Projections)** - Population broken out by 5-year age group, sex, and race/ethnicity, combining DoF P-3 projections for California's state, regions, and counties out to 2070 with Census cc-est estimates for the 50 states through 2025.
  - It is the module for questions about who a population is made of and how that composition is expected to shift - aging, racial and ethnic change, and cohort growth - and at 1.7 million rows it is by far the largest contract, so charts should filter before they aggregate.
- **ACS Housing Stress** - Housing cost burden from the Census ACS 1-year estimates (table B25140) for 2012 through 2024 (2020 is unavailable), reporting both the number and share of households spending over 30% and over 50% of income on housing, cut by tenure (renters, owners with and without a mortgage) and by race/ethnicity, for California's state, regions, and counties plus all 50 states.
  - It is the affordability measure of the set, and because it carries both tenure and race/ethnicity it supports equity comparisons that the other modules cannot make.
- **Building Permits** - Monthly residential building permits from the Census Bureau's Building Permits Survey, running 2010-01 through 2026-06 for 26 California metropolitan areas and all 50 states, with unit counts split by structure size (1 unit, 2 units, 3-4 units, 5 or more).
  - It is the leading indicator of new housing supply and the only monthly module, so it is the right source for recent turning points, though its metro geography does not line up with the county-based cuts used elsewhere.
- **RHNA Progress Report** - Progress toward state housing targets for all 539 California jurisdictions (cities plus unincorporated county areas) across the 5th and 6th RHNA cycles, reporting permitted units against each jurisdiction's allocation by income level (very low, low, moderate, above moderate, and total), with PPIC's pace and on-track scoring computed in the pipeline.
  - Each biweekly HCD release is captured as a dated `Snapshot Date` rather than overwriting the last one, so the module answers both "who is behind on their housing goals right now" and, as snapshots accrue, "who is falling further behind over time."
...
*Status as of 2026-07-16. **The five migrated modules are Verified across every audit column except Efficiency** — for which no dedicated performance pass has been logged for any module yet. Each module completed its as-built flagged-issue resolution (Reliability + Robustness, each with regression tests and an offline end-to-end re-verification) and the project lead's sign-off against the rendered site (the **Manually verified** column): PopHousing 2026-07-11, Components of Change 2026-07-13, Demographic Projections 2026-07-13 (guide items A1–A8, B1–B5), ACS Housing Stress 2026-07-13 (A1–A9, B1–B4), and Building Permits 2026-07-14 (A1–A7, B1–B4). Efficiency remains the single open audit column across the board for those five. **A sixth module, RHNA Progress Report** — the first net-new, non-legacy module — was added 2026-07-16: built **test-first** and green offline (115 backend tests; its frontend covered inside the 380-test Vitest suite), it is **Offline-Verified but not yet live-verified** (no live CKAN run has been logged), so its Reliability, Robustness, Live functionality, and Manual sign-off remain open (footnotes ² and ³). See the note beneath the table.*
...
| **RHNA Progress Report**     | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#E36A18;"></span>In Progress</span> | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#E36A18;"></span>In Progress</span> | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#9BA3A8;"></span>Not Started</span> | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#9BA3A8;"></span>Not Started</span> ² | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#084D7C;"></span>Verified</span> ³ | <span style="display:inline-flex;align-items:center;gap:6px;padding:2px 9px;border-radius:9999px;background:#edeff0;font-size:12px;white-space:nowrap;"><span style="width:8px;height:8px;border-radius:9999px;background:#E36A18;"></span>In Progress</span> |
...
¹ **Building Permits live functionality** — verified against Census BPS for the source's rolling ~2-year window (2024-01 onward); deep history (2010-01–2023-12) lives in the immutable, read-only `BuildingPermits_Historical.csv` seed, which the live run composes with each pull rather than re-fetching.
...
The entry point is [`scripts/orchestrators/projections_pipeline.py`](../../../scripts/orchestrators/projections_pipeline.py). `build_projections_dataset(config=None, logger=None)` runs five phases, each wrapped so any exception re-raises as a **`ProjectionsPipelinePhaseError`** tagged with the phase name. It returns a summary dict: dataframe, per-source *new-data* and *fallback* flags, output path (`None` when nothing changed), and row count.

| Phase | Name | What happens | Primary modules |
|---|---|---|---|
| **1** | Setup & Load | Resolve config; build the historical + fallback source by **unioning an immutable deep-history seed (`DemographicProjections_Historical.csv`, optional) with the current output**, preferring current rows. Absent both → cold-start on live data. | `config/*`, `merging/historical_merge` |
...
| **3** | Cleaning | `clean_p3_projections` maps FIPS→county (**dropping-and-logging** any non-county FIPS, then asserting all 58 counties remain), decodes `race7`, **bins single-year ages to 5-year groups (summing `perwt`)**, standardizes sex, tags `Geographic Level = County`. `clean_census_estimates` filters `SUMLEV=050`, sums counties to the 50 states, reshapes wide→long, **derives the calendar year arithmetically** (drop-and-log for out-of-range/unknown codes), tags `US State`. Each successful clean is gated through `validate_cleaning_output` before merge. | `cleaning/*` (+ shared `race_ethnicity_mapping`, `age_group_standardizer`) |
| **4** | Merge & Aggregate | Validate each incoming source/year's **stratification completeness** before any atomic replacement; **reduce retained history to base strata** so partial-year releases can't double-count; merge DoF + Census; build the 9 CA regions and the California state total by summing counties; add precomputed `All Ages` / `Both Sexes` / `All` rows; **detect new data by comparing base-vs-base grains** (so the flags are meaningful). | `merging/historical_merge`, `aggregation/regional_aggregation`, `aggregation/precomputed_totals` |
| **5** | Finalize, Validate & Save | Source-aware geographic-level assignment, enforce output column order, validate, and **atomically archive + save only when new source data was detected** (gated on the now-trustworthy flags). | `output/finalize_dataset`, `validation/projections_validators` |
...
| Script | Public functions |
|---|---|
| `dof_p3_downloader.py` | `get_p3_file_url`, `get_p3_file_url_positional`, `download_p3_data`, `extract_csv_from_zip`, `get_most_recent_p3_file`, `validate_p3_csv` (raw-header check catching duplicate columns before pandas mangles them — now **called in the live strategy** so a malformed download fails fast into the fallback cascade). |
| `census_ccest_downloader.py` | `get_census_ccest_url` (navigates the datasets index to the **latest `YYYY-YYYY` vintage**, then constructs `.../counties/asrh/cc-est<endyear>-alldata.csv` — the `asrh/` listing is too large to scrape), `download_census_ccest`, `validate_ccest_headers` (**called in the live strategy** to fail fast on a malformed download). |
...
| Script | Public functions |
|---|---|
| `validation/projections_validators.py` | `validate_cleaning_output` (**now wired** into each clean via the orchestrator's `_try_clean`, reading `critical_columns`), `validate_projections_dataset`, `validate_stratification_completeness` (base age × sex × race matrix per `Geographic Level × Location × Year × Source`, excluding the `All …` aggregate rows so totals can't hide gaps — the fail-closed backstop for the cleaners' drop-and-log). |
| `output/finalize_dataset.py` | `assign_geographic_level` (source-aware: California → `State` under DoF, `US State` under Census), `prepare_projections_output`, `archive_and_save` (**atomic `.tmp` + `os.replace`**; byte-identical skip; `mm-dd-yy` archive timestamp). |
...
| P-3 accepted horizon | soft sanity bounds `(2015, 2100)` + max span 60 (validated against the **observed** vintage, not a hard 2020–2070) | `schemas.py` |
| Census year decode | `year = census_year_base (2018) + code`, guarded by `census_year_sane_range` (derive-and-accept; a new vintage code ingests) | `schemas.py` |
| Immutable history seed | `DemographicProjections_Historical.csv` (optional, read-only; unioned with current output) | `paths.py` |
...
The module sources its California county/region names from the shared [`california_geography`](../../../scripts/shared/geography/california_geography.py) provider; the 58-county **FIPS map** and the 50 U.S. state names are projections-specific and live in `schemas.py`. The FIPS map is still built by the alphabetical-FIPS arithmetic rule, but the cleaner now **tolerates** a non-county FIPS (drop-and-log) while asserting all 58 counties are present, so a future summary/junk row degrades gracefully instead of failing the run.
...
3. **Idempotency.** An immediate second run detects no new source data and prints `No new data detected; file unchanged.` Since the 2026-07-13 pass this is enforced **two ways**: the base-vs-base change flags now correctly read `False`, so `archive_and_save` is never even called (the expensive full-file serialize + hash no longer runs), and the byte-identical-skip remains as a final guard — so re-running never churns the contract or the archive.
4. **Regression suite.** `python -m pytest` is green (**1,066 tests**, including new regressions locking each audit fix below), and `ruff check` is clean on every changed file.

Offline verification (`PROJECTIONS_OFFLINE=1`) is a full no-network end-to-end run from the local P-3 + cc-est files; the 2026-07-13 pass used it to re-verify the whole pipeline (1,718,208 rows, `dof_new=False, census_new=False` on the unchanged history, no write).
...
| Pipeline crash on any source failure | Saved-fallback returns *enriched* rows that Phase 4 **re-aggregated**, double-counting into 1.84M duplicate keys | `reduce_to_base_strata` (since 2026-07-13 the shared helper in `merging/historical_merge.py`) strips a saved fallback back to base strata before aggregation (see *the base-strata guard*) |
...
### As-built hardening pass (2026-07-13)

A later reliability/robustness/efficiency pass resolved every remaining flagged issue from the module's as-built guide, each with regression tests and an offline end-to-end re-verification. Two governance decisions set the posture: **derive-and-accept** for drift whose value is a stable arithmetic law (the Census year), and **degrade-gracefully for value-level surprises / fail-closed for structural breaches**.

| Issue | Was | Now |
|---|---|---|
| Change-detection grain (A1/B3) | Compared base-strata cleaned rows against *enriched* history, so the new-data flags were always `True` and `archive_and_save` serialized + hashed ~1.7M rows every run | Both sides reduced to base strata (shared `reduce_to_base_strata`); flags are truthful and the save (with its serialize + hash) is skipped when nothing changed |
| Census year decode (A2) | Capped `{code: 2018+code for code in range(2,8)}`; the first `cc-est2026` (code 8) would raise → stale fallback → frozen national series | `2018 + code` with a sane-range guard; a new vintage ingests, an out-of-range code is dropped-and-logged |
| P-3 horizon (A3) | Hard-coded `(2020, 2070)` in two places → a shifted vintage would fail cleaning → stale DoF | Validates the **observed** range against soft bounds `(2015, 2100)` + max span 60 |
| Dead validators (A4) | `validate_cleaning_output` never called and its null check read a non-existent `key_columns`; `validate_p3_csv` / `validate_ccest_headers` never called | All three wired (post-clean gate + download-time fail-fast); the null check reads `critical_columns` |
| Self-perpetuating history (A5) | `historical_data_path == current_data_path` | Distinct immutable seed `DemographicProjections_Historical.csv`, unioned with current; cold-start preserved |
| FIPS map brittleness (A7) | Any non-county FIPS raised → whole-file stale fallback | Drop-and-log non-county rows; assert all 58 counties present (structural fail-closed) |
| Retained-history double-count (A8) | Retained enriched rows re-aggregated (Region/marginal builders unguarded) | Retained years reduced to base strata before aggregation |
| Non-atomic write (B1) | `write_bytes` in place → a mid-write crash corrupts the contract that doubles as history | Staged `.tmp` + `os.replace` |
| Age-label drift (B4) | Two independent label routes with nothing enforcing agreement | Canonical labels are the single source; import-time parity assertion |

Value-level drift (unknown Census year/AGEGRP code, unrecognized race prefix, non-county FIPS) is **dropped-and-logged** so one surprise no longer discards the whole file, while the strict per-group 252-cell completeness gate is the **fail-closed** backstop for a genuine loss of a county, state, or year. Logging now covers the acquisition/cleaning/merge boundaries and records which fallback tier each source used.
...
The module is complete and runs end-to-end against both live sources. The **2026-07-13 as-built hardening pass** (see *Verification → As-built hardening pass*) resolved every flagged issue from the guide with tests; the full suite is green (1,066 tests) and `ruff` is clean on the touched modules. A few things about *today's* state are worth recording:
...
- **cc-est reshape rule.** `reshape_ccest_to_long` treats a `TOT`-prefixed `_MALE/_FEMALE` column as an ignorable total and, since 2026-07-13, **skips-and-logs** any other unrecognized race prefix rather than raising (value-level drift is tolerated; the completeness gate catches a genuine loss of a known race); in the real flow this is moot because aggregation already narrows to the 14 canonical race×sex columns.
...
**Date coverage:** 2010-01 → latest published month (currently 2026-05). Pre-2024 lives in the immutable `BuildingPermits_Historical.csv` seed (composed in each run); 2024-01 onward is live. Contiguity is enforced across the present span, not back to a fixed floor.
...
- **Verified run + immutable seed.** The live pipeline pulls 2024-01 onward; deep history (2010-01…2023-12) lives in a dedicated, read-only artifact `data/data-cleaned/building-permits/BuildingPermits_Historical.csv` (12,516 rows, originally seeded from the legacy accumulated snapshot). Phase 1 **composes** the seed with the live output (preferring live rows), giving `BuildingPermits_Current.csv` = **197 months, 14,691 rows**. The pipeline is idempotent on re-run (verified 2026-07-14: compose → prepare reproduces the exact 14,691 rows, byte-identical to disk, no change flagged).
- **Rolling source window + protected deep history.** The Census hosts only ~2 years of monthly `.xls` files, so a cold-start run cannot rebuild pre-2024 from the source. The immutable `BuildingPermits_Historical.csv` seed — which the pipeline **never writes** — is the durable system of record for deep history, and a Phase 5 guard fails the run loudly if any seeded month goes missing from the output; the contract file is written atomically so a mid-write crash cannot truncate it. `acquire_months` skips not-published months (routed through the logger); final-dataset contiguity is checked across the present span. *(Open: cross-check the seeded monthly totals against the Census annual BPS files, which reach back to 2010, for an independent validation.)*
...
| **Control rail** | [`workbench/ModuleSidebar.js`](../../../components/chart-builder/workbench/ModuleSidebar.js), [`resizableSidebar.js`](../../../components/chart-builder/resizableSidebar.js) | Renders whichever sections apply, in one accordion. On desktop it is absolutely positioned inside a `lg:relative` cell, so it contributes **zero intrinsic height** and the chart container alone sizes the row; overflow scrolls inside the rail. Its right edge uses the same persisted 288-680px drag resize control as the standalone tool. Below `lg` it returns to static flow. |
...
- **Floating actions** — a shared `fixed` bottom-right row in `app/layout.js` holds
  `ReportProblemDialog` (site-wide, labeled **"Feedback"** — opens the PPIC feedback
  form) and `BackToTopButton` (`components/documents/`). `BackToTopButton` only renders
  on `/documents` routes and only once the page has scrolled past `SCROLL_THRESHOLD`
  (400px), sitting to the right of the feedback button in the same row.
