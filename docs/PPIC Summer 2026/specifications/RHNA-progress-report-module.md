...
> [!danger] Two bugs let a same-day re-run duplicate every row, which double-renders the charts
> **Confirmed on the live runs of 2026-08-07.** Two runs 5 minutes apart left `RHNAProgress_Current.csv` at 16,170 rows with **5,390 of 10,780 grain keys duplicated**, and because the duplicates carry `Most Recent = True`, the frontend's default query path returned every jurisdiction twice. The offending data was reverted out of `main`; the two defects below are still live in the code.
>
> **1. The re-download gate compares mismatched granularities (the trigger).** `_latest_snapshot_by_cycle` (`orchestrators/rhna_progress_pipeline.py:90`) stores the last-seen snapshot **truncated to a date**, `latest[int(cycle)] = str(newest.date())`, while `_is_newer` (`acquisition/ckan_downloader.py:93`) compares the resource's **full timestamp** against it. `pd.Timestamp("2026-08-07 15:48:50") > pd.Timestamp("2026-08-07")` stays true for the rest of the day, so every cycle re-downloads on every subsequent same-day run. Fix either side; keeping the full timestamp when storing is the smaller change and preserves the ability to capture two genuinely different HCD revisions in one day.
>
> **2. The grain dedupe does not collapse a re-captured snapshot (the actual corruption).** This is the one that damages the data. Both runs read the *same* resource `last_modified`, so the re-captured rows have **identical** `(Jurisdiction, Cycle, Snapshot Date, Income Level)` keys - exactly what `union_snapshots` (`merging/historical_merge.py:66`) documents itself as de-duplicating on, "preferring the freshest capture." It did not. Per-cycle row counts show it plainly: the `2026-07-15` snapshot held the correct 2,695 rows per cycle, while each 2026-08-07 capture held 5,390. Fixing bug 1 alone would only hide this; a source that genuinely republishes under an unchanged `last_modified` would corrupt the dataset again.
>
> **A latent crash follows from the same data.** Once the column mixes `2026-07-15` with `2026-08-07 15:48:49.003781`, `mark_most_recent` (`enrichment/overall_progress.py:100`) calls `pd.to_datetime` with no `format=`, and re-reading the CSV raises `ValueError: time data "2026-07-15" doesn't match format "%Y-%m-%d %H:%M:%S.%f"`. Pass `format="ISO8601"` (or `"mixed"`), or normalize `Snapshot Date` to one representation on write.
>
> **Regression tests to add:** two runs in one day against an unchanged source leave the row count unchanged; `union_snapshots` given a frame that repeats an existing grain key returns one row for it; `mark_most_recent` parses a column mixing date-only and full-timestamp values.
>
