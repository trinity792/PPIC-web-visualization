...
- [Previous Tool: Technical Analysis \& Migration Guide](#previous-tool-technical-analysis--migration-guide)
  ...
  - [Shiny Web Application](#shiny-web-application)
    ...
    - [Shiny Visualization Topics](#shiny-visualization-topics)
...
The **Visualization Tool** consists of five standalone Python tools, each focused on one dataset: population/housing, components of change, demographic projections, building permits, and housing stress. Each topic scrapes its source, cleans it, merges it with a historical CSV, and draws interactive Plotly charts. People use these tools by running Jupyter notebooks.
...
> [!warning] 2. The Visualization Tool's Pop-Housing topic is county/region/state only
...
> It is the only topic built as a **class (`Projections`)**. Its ETL runs **once, in `__init__()`**, not on every chart call. Its `visualize_*` methods read the already-loaded `self.Projections` DataFrame and never re-scrape. `visualize_line()` is even **stateful** — it appends traces to a persistent `self.fig` so successive calls can overlay series.
...
Each topic is a self-contained scrape-clean-visualize unit for one dataset. They share a common shape — scrape a government source, reshape it, merge with a historical CSV, archive the old version, and expose `visualize_line()`, `visualize_bar()`, and (usually) `visualize_map()`.

> [!warning] The shared "gotcha"
> For four of the five topics, the ETL is **embedded inside** every chart function. The exception is [Age-Sex-Race Projections](#age-sex-race-projections). See [Key Architecture Facts](#key-architecture-facts).
...
> `Geographic Level` = `np.select(..., ['State','Region','County'], default='Other')`. There is **no City or Town**. This topic's `PopHousing_Current.csv` is a few thousand rows of aggregates — *not* the ~19,160-row city-level file the pipeline builds.
...
> [!note] This topic is structurally different from the other four
...
> [!warning] This topic is keyed by **month (`YYYY-MM`)**, not year — unlike every other topic. Its geojson files are loaded by relative path while its data CSVs use absolute `R:\UCF\...` paths, so it only works from a specific working directory.
...
> [!note] No map here
> This is the only topic with just two chart types (hence only two notebooks). Its data is **long-format**, stratified by `Race/ethnicity` and a tenure `Label`. Geographic levels: State (50 US states), County (CA), Region (9 CA regions).
...
These conventions recur across all five topics. Note where each one is *fragile* — those notes feed the [Robustness Risks](#robustness-risks--improvement-opportunities) section.

**ETL-inside-visualization.** For four of five topics, drawing a chart triggers the whole scrape → clean → combine → render → conditional-save sequence. Projections runs ETL once in its constructor instead.
...
**Geographic-level tagging.** Each script re-derives `Geographic Level` with `np.select()` just before saving. Conditions differ by topic; no Visualization Tool topic emits City or Town.
...
**Visualization notebooks (14).** Each topic ships 2–3 notebooks (bar, line, and usually map) that import the topic's `.py` file and call its chart functions interactively. These notebooks are the actual user interface for the Visualization Tool.
...
**Cleaning notebooks (6).** In `__cleaning__/`, these document the original data-prep work later formalized into the `.py` topics. They are the historical record of how each `*_Current.csv` was first built.
...
Pre-deployment checks: directory structure, installed dependencies, core package imports, data-directory access, and config loading.
...
### Shiny Visualization Topics

Three helper files feed the dashboard's overview/auxiliary visuals; all produce Plotly `go.Figure` objects embedded via `fig.to_html`.
...
A fourth, larger pipeline-side topic — **`production_pipeline/visualizations/advanced_city_analysis.py`** (~950 lines) — provides city-level growth classification and the shared brand palette referenced by `basic_visualizations.py`.
...
- [ ] **`warnings.filterwarnings("ignore")`** at the top of every Visualization Tool topic hides pandas dtype/parse warnings that would reveal bad inputs. *Improve:* scope or remove the suppression; surface warnings in logs.
...
| Brand palette (`#293B54`, …) | A shared theme/constants topic. |
