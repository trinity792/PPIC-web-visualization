---
Topic: Technical
Content Type: topic specification
pinned: false
description: "Technical topic specification for the RHNA Progress Report topic"
...
# RHNA Progress Report Topic Specification

> [!info] What this document is
> This is the **specification** for the RHNA Progress Report topic - a **new** topic with no legacy predecessor, so there is no migration path to trace; the document simply specifies the topic as it stands, kept current with the implementation (backend and frontend both landed 2026-07-16). It has two halves. The **first half is non-technical**: what the topic produces, where the data comes from, the dataset grain and columns, the geography, how the pipeline runs phase by phase, and the headline calculations. The **second half is a programmer reference**: per script, per function - what each does, the notable libraries it uses, efficiency notes, and the source fact or design decision behind it. The field catalog and grain were verified against the live CKAN resources, both data dictionaries, and the built pipeline's own output.

> [!warning] Status: built and green, not yet live-verified
> The backend (15 source files plus the orchestrator) passes **115 unit tests**, and the frontend is covered inside the repository's **380-test Vitest suite**; both are green and lint-clean. Two things stand between here and `Finalized`:
...
The topic publishes a single canonical dataset tracking each California jurisdiction's progress against its **Regional Housing Needs Allocation (RHNA)** - how many housing units have been permitted versus how many the state requires, split by the four income tiers (Very Low, Low, Moderate, Above Moderate) plus a Total. On top of the raw counts it computes PPIC's **pace analysis**: whether each jurisdiction is on track to meet its allocation given how much of its planning period has elapsed.

- **Source:** [data.ca.gov RHNA Progress Report](1), a CKAN package (id `ff082e96-72f7-4443-9747-8b8dadc15671`) published by the California Dept. of Housing and Community Development (HCD). The counts are the sum of units on jurisdictions' annual progress reports (APR Tables A and A3 for 2013-2017, Table A2 from 2018 on).
...
> The topic stores **two intentional lenses** on a jurisdiction's overall performance, and they are meant to disagree early in a cycle:
...
**Immutable seed:** `RHNAProgress_Historical.csv` holds any snapshots captured before the topic went live, read-only to the pipeline and unioned in during merge - mirroring the other topics' historical-seed pattern so a bad `Current` write cannot poison the accumulated series. It is empty today (history starts the day the topic goes live).
...
| # | Phase | Responsibility | Key topics |
|---|---|---|---|
...
2. **Chart editor** - `/rhna-progress` opens the shared `ModuleEditor` on the **cross-sectional ranking view** (`compare-places` bar), Income Level pinned in the sidebar. The topic schema declares `defaultPreset: "compare-places"` and `supportedChartTypes: ["bar"]`, and deliberately carries **no `yearRange`**, so the sidebar's temporal/period sliders stay hidden.
...
- `acquire_with_fallback(source_config, paths, latest_snapshot_by_cycle)` - runs the live -> manual -> saved-canonical ladder described above. On the live path it stamps each record's `source_last_updated` from the package `metadata_modified`. Imports the four `ckan_downloader` functions at topic scope so the tests can monkeypatch each rung. *Design fact: `source_failed` means "fell back to the saved canonical" (no fresh data at all), which is why the manual rung reports `False` for it.*
...
- Shared editor mechanisms added for this topic (general, other topics can use them): `schema.defaultPreset` (honored in `chartConfigStore.createChartConfig`) and `schema.supportedChartTypes` (gated in `wizard/steps/ChartTypeStep.js`).

---

## Flagged Issues and Fragilities

> [!success] Resolved 2026-08-07: same-day re-runs used to duplicate every row
> Kept as a record, because the failure mode was subtle and the shape of it is worth recognizing again. Two runs 5 minutes apart left `RHNAProgress_Current.csv` at 16,170 rows with **5,390 of 10,780 grain keys duplicated**, and because the duplicates carried `Most Recent = True`, the frontend's default query path returned every jurisdiction twice. Three defects compounded, all now fixed and covered by regression tests.
>
> **1. The re-download gate compared mismatched granularities (the trigger).** `_latest_snapshot_by_cycle` stored the last-seen snapshot truncated to a date while `_is_newer` compared the resource's full timestamp against it, so `"2026-08-07 15:48:50" > "2026-08-07"` stayed true for the rest of the day and every cycle re-downloaded. It now keeps the full timestamp.
>
> **2. The grain dedupe did not collapse a re-captured snapshot (the actual corruption).** `combine_snapshots` de-duplicated on the raw `Snapshot Date` column, but `existing` comes back from `read_csv` as strings while a fresh capture carries `pd.Timestamp`, so the same logical snapshot compared unequal and both copies survived. It now de-duplicates on a parsed key that is dropped again afterwards, leaving the stored representation untouched - the frontend sorts `Snapshot Date` with `localeCompare`, so it has to stay a stable string.
>
> **3. Date parsing inferred a format from the first value.** Every accumulated date column holds more than one shape, because rows written in different runs round-trip through CSV differently: `Snapshot Date` carries `2026-07-15` beside `2026-08-07 15:48:49.003781`, and `Planning Period Start`/`End` carry `2019-06-30` beside `2019-06-30 00:00:00`. With `errors="coerce"` that silently turned 5,390 rows into `NaT`, so `classify_status` could not see that a deadline had passed and reported a bogus `Status 'Behind' inconsistent with the four-quadrant rule`. All six parse sites now share `parse_date_column` (`enrichment/pace_metrics.py`), which passes `format="ISO8601"` and defaults to `errors="raise"` - the coerce variant is what turned a parse failure into a wrong answer rather than a visible one. Raw HCD parsing stays in `column_normalization`, which knows its exact `%m/%d/%Y`.
>
> **Verified live**: a fresh capture produced 10,780 rows with zero duplicated grain keys and 2,695 rows per (snapshot, cycle), and an immediate re-run acquired no cycles, wrote nothing, and left the canonical file's mtime untouched.
>
> Unrelated to the shared archive refactor, which touched only this topic's `output/finalize_dataset.py`. Found while verifying that work; see `docs/archive/shared-archive-and-save-guide.md` (archived implementation record).
...
- [ ] Promote the topic to `Finalized` in this doc and mark it verified in `projectSpec.md` once the live run is logged.
