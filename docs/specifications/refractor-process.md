...
pinned: false
description: "The process behind each topic refactor and the key decisions made along the way, with the reasoning that justified them. A narrative companion to projectSpec (the architecture reference) and the per-topic refractor guides (the per-function as-built docs)."
...
> This document is a **process and decision journal**. It records the *order* in which each topic was built, the *choices* made during the migration, the *reasoning* behind them, and the *surprises* that only surfaced when the pipeline first met live data. It is deliberately narrative.
>
...
> - The topic topic guides ([[population-and-housing-topic-guide]], [[components-of-change-topic-guide]], [[age-sex-race-projections-topic-guide]], [[acs-housing-stress-topic-guide]], [[building-permits-topic-guide]]) are the **per-function as-built** documents.
...
Every topic followed the same arc. The playbook was not designed up front; it emerged from **Population & Housing** (the first migration), was written down as a repeatable template, and each later topic refined it. By the time all five legacy datasets were migrated, the sequence below was routine.

### The seven steps

| Step | What happens | Output |
|---|---|---|
| 1. Analyze the legacy topic | Read the notebook / Shiny script, inventory every function, its data sources, and its fragilities. | A section of [[previous_tool_analysis]] and the "Legacy Topic Summary" of the topic's refractor guide. |
...
| 3. Write the tests (from the third topic onward) | For Projections, Housing Stress, and Building Permits the unit-test suite was written **first** and used as the authoritative contract. | A `scripts/unit_tests/<topic>/` tree of red tests. |
...
| 5. Wire the frontend | Add the topic schema, the `lib/data/<topic>.js` access layer, and the `app/api/<topic>/route.js` route; register the topic. | A live `/<topic>` chart-editor page. |
| 6. Run it against live sources | Take the pipeline off mocks and point it at the real upstream. **This is where every topic broke** — see each topic's "First live run" note below. | A verified end-to-end run and a set of regression tests. |
| 7. Audit and harden | Reliability, robustness, efficiency, and manual sign-off passes, tracked in the *Topic Audit Status* table in [[projectSpec]]. | Audit chips advanced from Not Started toward Verified. |

> [!note] Five canonical phases, PopHousing is the expansion
> PopHousing runs **six** orchestrated phases (it validates input up front and keeps Enrichment as a separate phase after Merge). The four topics built after it settled on **five canonical phases** — Setup & Load, Acquisition, Transform, Merge, Finalize/Validate/Save. PopHousing is framed as *its own expansion* of that five-phase shape, not as the template the others must copy. See the *Architecture Overview* in [[projectSpec]].

### Project-wide decisions that hold for every topic

These were settled once, early, and treated as requirements rather than preferences for the rest of the project.

| Decision | Justification |
|---|---|
| **Three-layer backend** (`shared/` mechanisms → `<topic>/` domain → `orchestrators/` sequencing), one-way dependency. | Keeps shared code project-independent and testable with generic fixtures; a topic can be understood and tested without reading any other topic. |
| **One cleaned CSV per topic is the contract.** The backend's only job is to produce it; the frontend's only input is to read it. | Decouples the ETL from the UI. Either half can be rebuilt without touching the other, and the contract is the single thing to validate. |
...
| **Writes are conditional.** The canonical CSV is re-archived and re-saved only when the data actually changed. | Prevents archive churn and keeps run history meaningful. (Change-detection got this wrong in more than one topic — see *Cross-cutting patterns*.) |
...
| **Documented for non-developers.** Each topic ships a two-half refractor guide: a plain-language half and a per-function programmer half. | The project has to outlive its authors and be legible to the researchers who use it. |
| **Test-first from topic 3 onward.** | Writing the contract as tests before the implementation caught schema drift early and made the phase-by-phase build mechanical. |

> [!warning] Mocked orchestrator tests hide integration seams
> The single most repeated lesson of the project: **a green test suite is not a working pipeline.** Every topic's orchestrator tests mocked the network and the file seams, so they passed while the real run crashed on the first live attempt. The gaps were always at the seams the mocks papered over — filename patterns, encodings, the shape passed between acquisition and cleaning, config keys the orchestrator read but the config never defined. Step 6 (a real run) is non-negotiable, and each topic below carries the specific reconciliation it needed.

---

## Population & Housing (PopHousing)

**Order: first.** PopHousing is the topic that *defined* the template. There was no playbook yet; the three-layer boundary, the contract-CSV idea, the phase structure, and the two-half documentation convention were all worked out here and then generalized.
...
| Put California county/region geography in **`scripts/shared/geography/`**, not in the topic. | It is reference data two topics genuinely share (PopHousing and, later, Components of Change), which is exactly what the shared reference-data provider is for. |
| Cross-topic run logging surfaced on **`/logs`** as structured JSONL + a per-run `.log`. | Operators need to see what each nightly run did without reading the server console. |
...
- **Self-perpetuating history.** `HISTORICAL_DATA_PATH == CURRENT_DATA_PATH`, so the orchestrator reads `PopHousing_Current.csv` as its own pre-2020 baseline and writes back to it. There is no immutable canonical E-8 file; the Phase 1 historical validator is the only guard. This pattern recurs in **every** topic.
...
**Order: second — the first topic built by *following* PopHousing's template rather than defining it.** This was the project's first evidence that the topic shape generalizes: someone could take the playbook and produce a working topic without re-deriving the architecture.
...
**Order: third — the first topic built entirely test-first.** The unit-test suite existed before any implementation and was read as the contract for each phase. This is the largest topic by row count.
...
The first topic to get a real performance pass: `assign_geographic_level` vectorized (3.29s → 0.21s on 1.72M rows); `archive_and_save` streams a SHA-256 hash + copies the file (instead of reading two full files into strings) for the byte-identity check (peak ~189MB → ~2MB); the canonical CSV is read once and threaded through; the DoF cleaner uses `usecols=` to drop a redundant 4.6M-row copy. The streamed-hash `archive_and_save` was later hoisted into the shared helper at `scripts/shared/archives/dataset_archive.py` (see `docs/archive/shared-archive-and-save-guide.md` (archived implementation record)), so this ~189MB→~2MB improvement is now shared behavior across all six writer topics, not a Projections-specific optimization. The frontend `loadProjectionsData()` parsing the whole ~87MB CSV per server process remains the open efficiency item.
...
| Reconcile the **9 ACS race iterations toward 7 canonical groups** plus "Other" and "All"; rename legacy `Label` → `Tenure`. | Aligns with the race grouping the other topics use while keeping the ACS-specific tenure dimension. |
...
A separate frontend bug surfaced here but affected **all** topics: navigating between topics (all under `/[module]`) reused one `ChartConfigProvider` instance, so the prior topic's field bindings lingered and failed validation against the new schema ("Resolve the configuration errors" on every preset). Fixed with `key={moduleId}` to remount the provider per topic.
...
**Order: fifth — test-first, and the first monthly topic. Its migration completed all five original legacy datasets.**

> [!success] Milestone (2026-07-03)
> With Building Permits migrated, **all five V1 legacy datasets run on the V3 architecture.** No original notebook dataset remains un-migrated; further work is enhancement, not net-new topic migration.
...
| Month-aware shaping (trailing-12 YTD, index-to-100, two-period change, region aggregate) lives in **`lib/data/building_permits.js`**, not shared `query_shapes.js`. | The shared render layer is year-centric; the monthly `Date` = "YYYY-MM" axis needs topic-local shaping. |
...
In **every** topic, `historical_data_path == current_data_path`. The orchestrator reads its own prior output as the historical baseline and writes back to the same file. There is no immutable canonical of record. It works because writes are conditional and validators guard the merge, but it means a corrupted or lost `Current.csv` cannot be re-derived — most dangerously in Building Permits, where the source cannot re-supply deep history.

### Stubbed logging

`scripts/shared/logging/pipeline_logging.py` and `dataframe_logging.py` have `pass` bodies and "Not yet implemented" tests across the whole project, even though orchestrator docstrings imply logging happens. The `/logs` feed is fed by the PopHousing JSONL path; wiring the shared logging package for every pipeline is outstanding work. (See [[logs notes]] for the intended log-card design.)
...
**Completed and signed off 2026-07-07.** Once all five topics were migrated, the frontend work shifted from per-topic wiring to a shared upgrade: turning the chart-builder into a Datawrapper-class editor. Full as-built detail is in [[visualization-specification]] (the 2026-07-07 guide is archived at [`archive/graphEditor-overhaul.md`](../archive/graphEditor-overhaul.md)); the process and the decisions are summarized here.
...
| **Topic-owned presets.** | Replaces the deleted `PresetPicker`; each topic declares its own presets that must pass validation, so Building Permits and Projections get bespoke chart shapes without a per-topic React component. |
...
Migration builds a topic; **auditing hardens it.** The *Topic Audit Status* table in [[projectSpec]] tracks each topic across six dimensions, using the shared design-system status chips (Verified / In Progress / Not Started).
...
The dimensions are orthogonal to migration status: a topic can be **Active** (migrated, live-verified) yet **un-audited** for robustness or efficiency. As of the last spec update, **Live functionality is Verified for all five topics** and **Offline functionality for four**, while the deeper Reliability / Robustness / Efficiency / Manual passes are mostly still Not Started — Demographic Projections is the furthest along, having received the first real reliability and efficiency passes on 2026-07-03.
...
- [ ] **Wire the shared logging package** for every pipeline; today only the PopHousing JSONL path feeds `/logs`, and `scripts/shared/logging/*` is stubbed.
...
- [ ] **Give each topic an immutable canonical of record** (or at least separate the read-baseline from the write-target) to end the self-perpetuating-history pattern — most urgent for Building Permits, whose deep history is irreplaceable.
