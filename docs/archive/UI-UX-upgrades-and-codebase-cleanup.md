...
- [x] Build metadata catalogue for each topic
...
### Reference Images
[[Dashboard Page.png]]
[[Detailed Topic Page.png|Detailed Topic Page.png]]
### User Flow
...
2. Clicks the category name (ie. "California Population and Housing Trends") or "See More ->" button
   - redirected to [[Detailed Topic Page.png|Detailed Topic Page.png]]
   - If clicked the see more button then the user is redirected to [[Detailed Topic Page.png|Detailed Topic Page.png]] with the contents of the graph/chart/table they clicked loaded.

# Codebase Cleanup
- [x] Create markdown-skill.md file
...
> **Source of truth for intent:** `docs/trinitys_notes/main.md` (line 64 onward — preset model, field catalog, dynamic sidebar, per-chart-type requirements, guardrails) and the **User Flow** + reference mockups above (`Dashboard Page.png`, `Detailed Module Page.png`).
...
| **API orchestrators** | `app/api/pophousing/route.js`, `app/api/components-of-change/route.js` | Thin. Validate query params, delegate to the data topic, shape JSON. Errors return `{ error, source }` with a non-200 status. |
...
- The client section components **duplicate** the curated parameter/subset lists that live in the server-only data topics (see the explanatory comment in `PopHousingLineSection.js`). There is no single client-safe catalog. → fixed by §2.
...
- `data/data-cleaned/california-counties.geojson` (58 county features, with `GEOID` + `NAME`) + `GET /api/geography?level=counties` (server topic `lib/data/geography.js`) — county map geometry, wired and tested, ready for the M4 choropleth.
...
| **Detailed Topic Page** | `app/[module]/page.js` (e.g. `/pophousing`) | The **editor**: dynamic sidebar + chart canvas + saved views. Clicking a category name opens it blank; clicking "See More →" on a specific tile opens it with that chart/config **preloaded** (via route param or `?view=` query). | `Detailed Module Page.png`, `src/imports/DetailedModulePage` |
...
lib/visualization/   (client-safe: NO node:fs)
  ...
  categoryRegistry.js    landing-page categories → topics (§6.1)
  moduleSchemas/
...
## 2. Visualization core — topic schemas & field catalog

**Checklist item:** "Build metadata catalogue for each topic."

Create the **client-safe** schema layer — the single source of truth for what fields/metrics/subsets exist and how they combine. It must not import `node:fs`, so both client components and the server data topics can consume it.
...
Port the `POPHOUSING_FIELDS` catalog from main.md (Year, Location, Geographic Level, Total Population, Total Housing Units, Vacancy Rate (%), …) and the Components-of-Change catalog (Births, Deaths, Natural Increase, Net Migration, crude rates, …). Each topic exports:
```js
...
```
**De-duplication:** `lib/data/pop_housing.js` and the client components should both import `curatedMeasures`/`subsets` from here instead of re-declaring them; `NUMERIC_COLUMNS` in the data topic derives from the schema's measure fields. Removes the duplicated lists flagged in §0.1.
...
**Acceptance:** schemas import-clean in a client component (no `node:fs`); the server data topics consume the same catalog (one definition of the curated metric list).
...
## 4. Chart-builder UI (the dynamic sidebar) — lives on the Detailed Topic Page
...
Under `components/chart-builder/`, each section generated from the active chart type's `sidebarSections` + the topic field catalog:
...
Implement main.md's line-layer model. A layer is `{ id, type, x, y, splitBy, values, filters, label }`. Allowed `type`s: `selectedPlaces`, `benchmark`, `secondSource` (Components of Change only), `secondMeasure` (same `comparisonGroup` only), `referenceValue`, `derivedComparison`. `LayerEditor` only offers types valid for the active chart/topic; `validation.js` gates `secondMeasure` on comparability.
...
## 6. App shell, landing & topic pages
...
### 6.3 Detailed Topic Page — `app/[module]/page.js`
- Route param selects the topic schema (`pophousing`, `components-of-change`). Layout = `ChartSidebar` (left, `ui/sidebar` + `ui/sheet` for mobile via `use-mobile`) + chart canvas (right) + saved-view bar.
- Read `?view=` (or param) and hydrate the config store via `LOAD_VIEW` to preload the clicked chart; otherwise open the topic's default preset.
...
The current API serves only `queryLineSeries`. New chart types need new server query shapes in `lib/data/*.js` + new (or parameterized) routes. Keep routes thin; keep all filtering in the data topic (mirrors the existing pattern).
...
- §2 topic schemas + field catalog; de-duplicate curated lists shared with `lib/data/*`.
...
- §6 `categoryRegistry`, landing category grid (`/`), Detailed Topic Page (`/pophousing`, `/components-of-change`) hosting the editor, Navbar rebuild.
- **Acceptance:** landing → topic page flow works; the two current line charts are reproduced entirely through preset → sidebar → config → renderer with working save/load/export.
