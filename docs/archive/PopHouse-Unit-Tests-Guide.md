...
pinned: false
description: "Guidelines and requirements for writing pytest tests covering the Phase 1 (Setup & Validation) and Phase 2 (Data Acquisition) topics of the refactored Pop-Housing pipeline."
...
Guidelines and requirements for writing pytest tests covering the Phase 1 (Setup & Validation) and Phase 2 (Data Acquisition) refactored topics.

---

## Test Directory Structure

Tests mirror the source tree. Each source file gets a corresponding test file in the same relative position under `scripts/unit_tests/`.
...
Build test DataFrames inside each test or in the topic's `conftest.py`. Keep them minimal — the fewest rows and columns that exercise the function's logic. Don't load real CSV files from the project.
...
### Config Fixtures — From the Active Config File

Tests should import configuration from the topic used by the source under test. Phase 1 and Phase 2 use `scripts/pophousing/config/paths.py` and `scripts/pophousing/config/sources.py`. Topics that still depend on Population & Housing geography or schema constants may import those values from `lib/pophousing_config.py`. Don't duplicate configuration values in test code — that's how drift happens.
...
| Test | What it verifies |
|---|---|
| `test_get_paths_project_root` | Resolves the repository root from the topic location. |
...
This table lists the minimum number of distinct test functions per topic. The actual count will be higher once you add edge cases that emerge during implementation.
...
Phase 3 is the most function-dense phase. It spans four shared topics and five pophousing topics. The tests are organized below in dependency order: shared utilities first, then pophousing topics that compose them.
...
Phase 5 adds one new function to the existing shared validators topic.
...
This topic contains the most intricate logic in Phase 3. The E-5 workbook uses a hierarchical layout where county names appear as headers above their constituent cities. Testing requires DataFrames that reproduce this layout.
...
This topic has the most functions of any Phase 3 topic. Most are small and focused, but `classify_ambiguous_location` and `assign_missing_geographic_levels` have significant branching.
...
Duplicates San Francisco rows so it appears as both a City and a County. This function runs exactly once in Phase 6, but it's defined in this topic and tested here.
...
This topic validates the output of the Phase 3 cleaning pipeline. The specific functions depend on your implementation, but the following scenarios must be covered:
...
Phase 4 is the smallest phase. All functions live in one pophousing topic.
...
The new shared topics (`type_conversions.py`, `row_filters.py`, `dataframe_operations.py`, and the `validate_numeric_range` addition) must be tested with generic column names and data. Use `"a"`, `"b"`, `"value"`, `"date"` — not `"Location"`, `"Total Population"`, `"Vacancy Rate"`. The pophousing tests are where domain-specific column names appear.
