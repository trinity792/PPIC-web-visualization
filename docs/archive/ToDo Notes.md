---
Topic: Other
Content Type: Notes
pinned: false
description: Notes for me to stay on track
Date Published: July 4, 2026
Last Updated: 09/18/2026 - 03:08 PM
Status: Archive
---

# To Do Notes
- [ ] Verify that all config options get carried over to export (line spacing, colors, font sizes, etc.)
- [ ] Gradient color options are ugly. Not true to color and seemed washed out.
- [ ] On forest/whisker plot, line of no effect is non functioning (seems to have regressed since after the viz tool changes)

- [ ] Provide more degrees of freedom for what values are allowed to be X axis or "Outcome"
- [ ] Modify the transpose option so that the column type indicator moves as well for imported data
- [ ] Bring back multi chart ability to topics

---

# Follow Ups
## A Running List
- Combined SVG/PDF are raster-wrapped, not true vector (combining vector charts is substantially more work). Single-chart SVG/PDF stay vector-first. If crisp vector multi-chart output matters, that's a follow-up.

---

## Structured ToDos
- [ ] Add a last updated footer to website, should include date and time in PT.
  - [ ] Full control over axis labeling
  - [ ] Excel like csv editor features (especially grouping & sorting)
- [ ] Parcel / Terner

- AI Comparison Guide Updates:
  - [ ] Skill files
  - [ ] Workflows
  - [ ] VSCode on MoWos
### Frontend
- [ ] Need to strip identifying information from the logs. Should use relative paths.

---

## Other Fun Ideas
- [ ] Tracking for how often PPIC's work is referenced in CA state legislature w/ identification if possible -> replace tool Gov Affairs pays for & create visualization
- [ ] Economy stats
- [ ] Climate/Water stats
- [ ] Document edit mode that allows pushing to git
- [ ] Map geojson library

---
# Internship Feedback
- [ ] In person coffe chat option

---
