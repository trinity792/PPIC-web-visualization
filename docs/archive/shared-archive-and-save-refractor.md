...
Last Updated: 09/18/2026 - 03:08 PM
Status: Archive
...
A cross-cutting refactor rather than a topic migration: five near-but-not-quite-identical archive-and-write implementations collapse into one shared mechanism, and the archive filename changes to `{topic}_{prefix}_{YYYY-MM-DD}.csv`.

> [!warning] Superseded, kept as a record
> This is the August 4 plan. It was replaced by `docs/archive/shared-archive-and-save-guide.md` (archived implementation record), which is the plan that was actually implemented and is now the as-built guide for the shared helper. **Do not act on anything below.** The filename format, the `module_id` rule, the `housing_stress` backfill resolution, and the treatment of `pophousing` as a sixth write path all reached their final form in the successor document. This one is preserved because its Open Questions and their answers are the record of how those decisions were reached.
...
| Topic | Function | Archive filename produced | Change detection | Copy or move |
|---|---|---|---|---|
...
Others assert on `archive_directory.glob("*.csv")` and on counts rather than names, which will survive a rename unchanged. Every test file in the table below still needs reading before its topic is touched, because a passing glob assertion can hide a name assumption in a fixture.
...
A new topic, `scripts/shared/archives/dataset_archive.py`, holding one public function. It belongs beside `file_retention.py` in the same package, which already owns archive mechanics.
...
def archive_and_save(dataframe, current_path, archive_directory, module_id, already_compared=False):
    ...
    **copied** to the archive, never moved, so a crash between archive and write can
    never leave the topic without a canonical CSV.

    Args:
        dataframe: the prepared output frame.
        current_path: canonical CSV path, e.g. data/data-cleaned/<topic>/X_Current.csv.
        archive_directory: data/archive/<topic>/.
...
The write sequence is the one four of the five topics already use, and is unchanged: archive the old file by copy, stage the new data to a sibling `.tmp`, then `replace()` it into place.
...
`module_id` is passed explicitly rather than derived from `archive_directory.name`. Deriving it would need no new plumbing and would usually be right, but it would silently produce `tmp_path`-derived garbage under tests and would couple the filename to a directory layout that is not guaranteed. Passing it explicitly also means the archive filename and the run record in `logs/pipeline-runs.jsonl` are provably the same string, since both come from the orchestrator's topic metadata.

> [!warning] The day-granularity collision survives this change
> `{YYYY-MM-DD}` has the same resolution as `{mm-dd-yy}`. Two refreshes of the same topic on the same day still produce the same filename, and the second overwrites the first. This is deliberate: the format stays readable, and the collision does not matter while refreshes are manual and roughly monthly. If a schedule is ever added, the fix is to extend the format to `{YYYY-MM-DD}T{HHMMSS}` in one place, which is the entire point of hoisting the function.
...
| `projections` | Archive name loses `_Current`, gains topic prefix and ISO date | None beyond the rename. |
| `housing_stress` | Archive name loses `_Current`, gains topic prefix and ISO date; **the backfill's seed archive needs its own `module_id`** | Second-highest. The rename itself is free, but the prefix rule maps `HousingStress_Current.csv` and `HousingStress_Historical.csv` onto one filename, so the backfill must pass `module_id="housing-stress-backfill"` or the seed archive is silently overwritten. See [§ 3](#3-housing_stress). |
| `components_of_change` | **Move becomes copy**; gains byte-level change detection; archives gain dates; counter-suffix disambiguation disappears | Highest. Four changes at once, and the only topic whose write path is genuinely re-specified. |

---

## Per-Topic Migration

Each topic is one commit. Order is chosen so the riskiest change lands last, against a helper already proven by four topics.
...
The fix is to apply the `module_id` rule literally rather than per-topic. The backfill's own `execute_pipeline_run` call (`housing_stress_backfill.py:226`) already passes `module_id="housing-stress-backfill"`, so passing that same string to `archive_and_save` yields `housing-stress-backfill_HousingStress_{YYYY-MM-DD}.csv`, distinct from the pipeline's `housing-stress_HousingStress_{YYYY-MM-DD}.csv` and self-describing about which driver produced it. This is what the helper's docstring already prescribes ("the same string the orchestrator passes to `execute_pipeline_run()`"), and it is the reason the argument is passed explicitly instead of derived from `archive_directory.name`, which would collapse the two.
...
`write_dataset` keeps its name and its `new_snapshot` guard, but its body delegates to the shared helper with `already_compared=True`. This is the only topic whose public interface stays as-is.
...
`write_components_output` was checked and does only an atomic `.tmp` plus `replace()` write, with no topic-specific work, so the shared helper replaces it exactly. Whether the function stays for other callers or is deleted is a judgement call at the time.
...
| `test_archive_filename_uses_module_prefix_and_iso_date` | Name matches `{topic}_{prefix}_{YYYY-MM-DD}.csv`. |
...
`scripts/unit_tests/{building_permits,projections,housing_stress,components_of_change,rhna_progress}/output/test_finalize_dataset.py`. Most assert via `glob("*.csv")` and counts and will pass unchanged; the ones that assert names need the new format. Each topic's tests are updated in that topic's commit, never ahead of it.

### Verification per commit

```bash
python -m pytest scripts/unit_tests/shared/archives/ -q          # after step 1
python -m pytest scripts/unit_tests/<topic>/ -q                  # after each topic
ruff check .
python -m pytest                                                  # before the final commit
```

> [!warning] Green tests are not a working pipeline
> [[refractor-process]] records this as the single most repeated lesson of the project: every topic's orchestrator tests mocked the file seams and passed while the real run crashed. The archive path is exactly such a seam. Each migrated topic needs one real run against live sources, confirming an archive file actually appears with the new name, before the topic is considered done.
...
- [ ] Migrate `building_permits`, update its tests, run the topic suite.
...
- [ ] One live run per topic, confirming archive filenames on disk.
...
|---|---|---|
| Filename format | `{topic}_{prefix}_{YYYY-MM-DD}.csv` | Self-identifying when moved off the drive; sorts chronologically in a listing. |
| Where `module_id` comes from | Explicit argument from the orchestrator | Avoids coupling the filename to directory layout, and matches the run record's topic string exactly. |
| Comparison strategy | Streamed SHA-256 for all topics | Already the optimised form in two topics, with a recorded 189MB to 2MB improvement. |
| Copy or move | Always copy | A crash must never leave a topic without a canonical CSV. |
