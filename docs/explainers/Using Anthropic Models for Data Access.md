...
pinned: false
description: "A durable guide to configuring Claude for repeatable web research, data comparison, and auditable results with project instructions and task skills."
...
# Using Claude for Repeatable Data Access

> [!warning] Product details change
> Claude product surfaces, model names, prices, and limits change more quickly than the workflow in this guide. Confirm current capabilities in Anthropic's product documentation and use [Anthropic's system cards](https://www.anthropic.com/system-cards) when a model-specific decision matters.
...
The appendix compares Claude.ai, Claude Code, and Claude Cowork. It also compares the Opus and Sonnet model roles and explains how to preserve that selection rule when release names change. The appendix provides writing rules for Markdown and Python files.
...
| Appendix | Claude products, model roles, selection criteria, and supporting writing rules |
...
Sonnet is the balanced model role for high-volume, well-defined work. Its speed and usage profile make it suitable for repeated source checks, routine implementation, and changelog entries. Opus is the higher-capability model role. Use it when a task requires more judgment, such as an unfamiliar site structure, an unclear instruction, planning a high-consequence change, or reviewing work where an incorrect result creates significant risk.

Use Opus to plan work that requires judgment, including adding a source or changing significance rules. Use Sonnet for routine implementation and repeated comparisons. Document these as capability roles rather than binding the workflow to a numbered release: “use the current balanced model for routine comparisons; escalate to the highest-capability approved model when the source structure changes or confidence is low.” Verify the current lineup in [Anthropic's system cards](https://www.anthropic.com/system-cards).

Before finalizing `## Verification`, ask a person to review consequential output. A second-model pass can find omissions, but the model that performed the task must not be the only reviewer, and model review does not replace checking the original sources.
