...
|---|---|---|---|
| **Running** | Executing each topic's pipeline: downloading the source data, cleaning it, and merging it with the existing dataset. | On an individual maintainer's personal device. | No |
...
**Updates require a person in the loop at every step.** The purpose of the topics is to give readers quick access to datasets that are refreshed at varying frequencies. Delivering on that today requires the repository to be cloned onto someone's machine, after which there are two ways for new data to reach the website. Either that device runs its own local automations to execute the pipeline and then push and commit the results, or a person performs those steps manually for each topic every time a source dataset is updated. The first option makes the project dependent on one person's machine staying configured and switched on. The second is sustainable at a handful of topics but grows costly as the topic count rises.
...
This option is viable at the current scale. With a handful of topics and a maintainer who knows the pipelines, updates reach the site within a day or two of a source release.
Its costs are staff time and continuity. Each refresh requires a person to notice the release, run the pipeline, review the output, and commit. That cost scales with the number of topics, and the ability to do it at all depends on one person's machine remaining configured and one person remaining available. Dataset files also grow with every refresh, so local disk usage increases indefinitely.
If the questions in [Appendix C](#appendix-c-open-questions) make the other options impractical, this is a workable steady state rather than a failure. The judgment call is how many topics PPIC expects to run and how quickly readers need new data after a source release.
...
This option solves all three pillars in a single move and gives PPIC full control over both access and data. That control is what would make it possible to build topics on proprietary datasets that cannot leave the organization's network.
However, this would likely increase maintenance requirements and complicate any path to a future public-facing version of this site. Serving over `https` would probably require setting up a [reverse proxy](1), and the whole arrangement depends on hardware and policy questions that have not yet been answered (see [Appendix C](#appendix-c-open-questions)).
...
| Suitable as a permanent arrangement | Yes, at a modest topic count. | Yes. | Yes. | No. Interim only. |
| Principal unknown | How many topics the manual process can carry. | PPIC server capabilities and policy restrictions. | Where processed data is permitted to live. | Whether a spare machine can be spared and where it sits on the network. |
...
This is a preference from the project side rather than an assessment of what is feasible or advisable given PPIC's infrastructure and policies. Option A remains fully functional if none of the others are approved, and the project can continue on it indefinitely at the current topic count.
