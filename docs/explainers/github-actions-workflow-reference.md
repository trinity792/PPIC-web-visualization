...
pinned: false
description: "Unimplemented reference plan for automating the data pipelines with GitHub Actions while keeping deployment and future PPIC infrastructure changes separate."
...
> This document remains a plan. As of September 18, 2026, the repository has no `.github/workflows/` directory. Commands and YAML below are proposed work, not a description of automation currently running.
...
## Stage 0: Verify the Local Environment

The repository's current local environment, `web-viz-venv`, runs Python 3.12, and `[tool.ruff]` in `pyproject.toml` targets `py312`. The proposed workflows below also request 3.12. Nothing *declares* a floor, because `requires-python` is a `[project]` field and this repository deliberately has no `[project]` table (see [Appendix B](#appendix-b-dependencies-and-why-they-are-declared-as-groups)).
...
Keep the local and runner environments on 3.12 so that a failure on the runner is reproducible on the MacBook. Rebuild the environment only when its interpreter or dependency groups are out of date.

```bash
cd /path/to/web-data-visualization
python3.12 -m venv web-viz-venv
source web-viz-venv/bin/activate   # Windows: web-viz-venv\Scripts\activate
...
The point of this stage is that from here on, "it works on my machine" and "it works on the runner" mean the same environment, which is what makes every later checkpoint diagnostic.
...
printf 'PPIC data-visualization archive.\nHolds gitignored pipeline output. Not the published dataset.\nSee docs/explainers/github-actions-workflow-reference.md\n' \
  > "$PPIC_ARCHIVE_DEST/README.txt"
```

Three subdirectories rather than one, because the destination answers two different questions and they want different shapes. `archive/<dir>/` accumulates every superseded CSV across all runs, answering "what did this dataset look like in May." `refresh-runs/<id>/` holds one bundle per run with its log and change report, answering "what happened on that run." Keeping them apart means the archive stays browsable by topic and date rather than buried under run identifiers.
...
└── workflows/
    ...
    ├── building-permits.yml      # inputs for one topic
    └── tests.yml                 # pytest and vitest on every pull request
```

The logic lives once. Each topic gets a caller of roughly twenty lines, so adding the seventh topic means writing a seventh caller rather than copying a hundred lines of YAML. Every construct used below is explained in [Appendix A](#appendix-a-construct-reference); the annotations here are limited to what affects whether the file works.

### The topic directory input, which is not the topic identifier

This repository has one naming inconsistency that will silently break the artifact steps if it is not accounted for, and it will not break them on the first topic you test.

`module_id` is the string the orchestrators pass to `execute_pipeline_run()` and `archive_and_save()`. It prefixes archive filenames and keys the run record. The *directory* each topic reads and writes under `data/data-cleaned/`, `data/archive/`, and `data/data-raw/` is a separate name, and for four of the seven topics the two do not match.

| Topic | `module_id` | Data directory | Same? |
|---|---|---|---|
...
The inputs are the per-topic knobs. `on: workflow_call` makes the file callable by other workflows rather than triggered by an event of its own.
...
The orchestrators append a structured run record to `logs/pipeline-runs.jsonl` on every run, success or failure. `scripts/shared/logging/change_report.py` selects the newest record for this topic and renders it as the Markdown a reviewer reads, so the workflow step is one line to two destinations.
...
> [!warning] The seven topics do not share a `result` schema, which is why this is not a heredoc
> An obvious inline script reads `result["new_data"]` and `result["source_failed"]`. Only two of the seven topics emit those keys. `rhna-progress` uses `new_snapshot`, `components-of-change` uses `new_census_data_found` and `new_dof_data_found`, `projections` uses `census_new_data` and `dof_new_data`, and `pophousing` emits no new-data flag at all. A `.get(key, "n/a")` swallows every one of those differences silently, so the reviewer reads `n/a` and cannot tell whether that means "nothing happened" or "this topic never reports it."
>
> Only `row_count` and `output_path` appear in all seven. The topic therefore leads with those, treats `output_path: null` as the uniform "nothing was written" signal, and discovers the topic-specific flags by pattern rather than from a per-topic table, so a topic added later reports what it emits instead of regressing to `n/a`.
...
If the source has published nothing since the last commit, the pipeline succeeds and opens no pull request. That is correct behaviour rather than a failure. To get something reviewable on a first pass, run against a topic whose data is genuinely stale.
...
> Wait for the Vercel deployment to finish, then load the topic's page and confirm the newest date in the chart matches what the change report said was added. This is the only checkpoint that verifies the whole chain end to end, and it is the one worth repeating after any later change to the workflow.
...
> [!warning] Two runs of one topic on the same day overwrite each other's archive entry
> `build_archive_filename()` stamps the archive name with `date.today()` and no run identifier, so two refreshes of the same topic on the same day produce the same filename and the second silently replaces the first. This is invisible on the destination, because the second download simply overwrites the first file.
>
...
>
> The fix is already half-built. `build_archive_filename()` accepts an optional `time_of_day` argument that appends `THHMMSS` and still sorts chronologically as a plain string; `archive_and_save()` simply never passes it. Passing it is a one-line change plus the docstrings that record the reasoning, and it changes the archive filename format for all six topics, so it is worth doing deliberately rather than as a side effect of enabling a cron. Treat it as a precondition for any cadence more frequent than weekly.

---

## Stage 8: Script the Sync

Three download lines retyped every month is three chances to typo a path or a run identifier. Per the project's Python conventions this belongs in the repository rather than in someone's shell history, as `scripts/shared/storage/sync_artifacts.sh` taking the topic as its argument and reading `PPIC_ARCHIVE_DEST` from the environment.
...
>
> Making it take a list of topics, so a monthly catch-up is one invocation, is worth the extra ten lines.
...
## Stage 9: Add the Remaining Topics

Only once building permits has been through a full cycle including a merge and a sync. Each additional topic is one caller file, and the values are the only thing that varies.

| Topic | `module_id` | `data_dir` | `orchestrator` |
|---|---|---|---|
...
| 2 | Actions tab, "Run workflow" on the topic, if triggering by hand | Browser |
...
- [ ] Stage 0: verify the Python 3.12 environment and confirm a clean pipeline run before implementing workflows. The interpreter is aligned; the full checkpoint still must be run.
...
- [ ] Estimate monthly Actions minutes across all topics, needed before the repository goes private.
...
**Should a failing pipeline block its own schedule?** As written, a topic that fails every Tuesday opens an issue every Tuesday. A backoff, or a check that suppresses duplicate issues, is easy to add but needs a policy decision about how loud repeated failure should be.
...
> `scripts/shared/logging/run_records.py` builds `PACIFIC = ZoneInfo("America/Los_Angeles")` at topic scope rather than inside a function. Linux and macOS ship an IANA timezone database; Windows does not. On a Windows machine that line raises `ZoneInfoNotFoundError` at *import* time, so every topic that imports the logger dies before `main()` is reached and no pipeline gets far enough to log why. The environment marker installs `tzdata` only where it is needed, so it costs nothing on the Ubuntu runner and saves any future move to a Windows machine.
...
| `AttributeError: 'RuntimeError' object has no attribute 'add_note'` | Python older than 3.11 | `python --version`; rebuild `web-viz-venv` per stage 0 |
...
| Change report warns that no record matched the topic | `module_id` in the caller does not match what the orchestrator passes | The mapping table in stage 2 against `execute_pipeline_run()` |
...
| Archive artifact empty for one topic | Artifact path keyed on `module_id` instead of `data_dir` | Compare against the mapping table in stage 2 |
