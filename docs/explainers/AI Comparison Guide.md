...
For an initial research workflow, this guide recommends either Perplexity Pro with the free version of Gemini Notebook for source gathering and source-based synthesis, or a paid Claude subscription with free ChatGPT access for research, drafting, and revision. Monthly subscriptions are the simplest option for individual users, while usage-based APIs are better suited to controlled, auditable automation when an organization has the technical capacity to manage them.
...
Claude (large context window, strong document analysis), ChatGPT Deep Research, Gemini Deep Research, Google Gemini Notebook (designed specifically for working with uploaded sources)
### Everything Else
...
- Gemini Notebook (described below) is a free, standalone tool for source-based analysis that has no direct equivalent from other providers.
...
- Google's AI product lineup is fragmented across Gemini, Gemini Notebook, AI Studio, Vertex AI, and Workspace integrations, which can be confusing when trying to determine which tool fits a specific need.
#### Privacy
...
Gemini Notebook is a separate Google product designed specifically for working with uploaded source material. You provide documents (PDFs, Google Docs, web links, YouTube videos), and Gemini Notebook indexes them into a "notebook" that you can query, summarize, and cross-reference. It does not search the open web; it only works with the sources you give it.
Gemini Notebook is free to use. It is particularly strong for literature reviews, comparing policy documents, and building structured summaries from a defined set of sources. Its "Audio Overview" feature can generate a podcast-style discussion of your uploaded materials.
### Gemma
...
## OpenAI
### ChatGPT (GPT Models)
ChatGPT is OpenAI's general-purpose AI product for conversational work, research, file analysis, content creation, and coding. Individual plans serve casual and power users, while Business, Enterprise, and Edu plans add organizational controls. The OpenAI API is a separate, usage-billed product for developers and automated workflows.
#### Model Tiers
- **GPT-6 Astra** is OpenAI's most capable model for difficult, multi-step work across code, applications, and research.
- **GPT-5.6 Sol** is designed for complex, open-ended professional work that benefits from deeper analysis and polish.
- **GPT-5.6 Terra** balances capability, speed, and cost for everyday work.
- **GPT-5.6 Luna** is the fastest and least expensive GPT-5.6 tier, intended for clear, repeatable, or high-volume tasks.
Model availability depends on the user's plan, client, sign-in method, and rollout stage. Users can also adjust reasoning effort: more reasoning can improve complex results but takes longer and consumes more usage. OpenAI has announced that GPT-5.5 will retire from ChatGPT, ChatGPT Work, and Codex on October 14, 2026; the retirement does not apply to the API.
ChatGPT includes capabilities beyond the base model, including first-party web search, deep research, voice, image generation and input, file analysis and creation, projects, memory, and plugins. Availability varies by plan and workspace settings.
#### Pros
- Broad built-in toolset spanning web research, file work, document creation, image generation, voice, coding, and external integrations.
- Multiple model and reasoning settings let users trade speed and usage for more complex analysis.
- First-party web search can provide current information and citations, while deep research can investigate across multiple sources and produce a reviewable report.
- Works as a standalone workspace rather than requiring a particular office suite.
- A free plan is available for basic use.
#### Cons
- ChatGPT can still produce incorrect facts, citations, or interpretations, so important claims and generated analysis require independent review.
- Model access, tools, and usage limits vary by plan and may change during staged rollouts.
- Higher reasoning settings and agentic features take longer and consume more of a plan's usage allowance.
- On individual accounts, conversations may be used to improve models unless the user disables that setting or uses Temporary Chat.
#### Privacy
On individual ChatGPT accounts, users can turn off **Improve the model for everyone** under Settings > Data Controls; Temporary Chats are not used for training and are deleted after 30 days. OpenAI does not train on Business, Enterprise, or Edu workspace data by default. API data is not used to train models unless the customer explicitly opts in. API abuse-monitoring logs are generally retained for up to 30 days, while application-state retention varies by endpoint and configuration; eligible API customers can apply for additional retention controls.
#### Costs
Current individual plans include Free (\$0/month), Go (\$8/month), Plus (\$20/month), and Pro starting at \$100/month. ChatGPT Business is \$20/user/month when billed annually or \$25/user/month when billed monthly, with a two-user minimum. Enterprise and Edu pricing is custom. API usage is billed separately from ChatGPT subscriptions.
#### Recommended Uses
General-purpose assistance for writing, research, brainstorming, file analysis, document creation, and coding. Astra and Sol are suited to difficult work requiring sustained reasoning; Terra is a strong default for everyday tasks; and Luna is better for focused, repeatable, or high-volume work. For research, review the cited sources and verify consequential claims independently.
## Anthropic
### Claude
...
# Recommendations
### Option 1:
Begin with a Perplexity Pro subscription + the free version of Gemini Notebook. Perplexity Pro works very well for research gathering, and its deep research mode is truly effective. Use Gemini Notebook for even more in-depth explanations and different ways of presenting information. Gemini Notebook can generate audio overviews, videos, reports, slide decks, and infographics based on the information you give it. Only upgrade to a paid plan (Google's AI plan) if you begin hitting source size or count limitations.
### Option 2:
