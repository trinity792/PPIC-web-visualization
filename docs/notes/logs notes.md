...
**Card layout:**
```
[Icon]  <Topic> — <one-line plain-English summary>
        When:   <date, time PST>
...
[AlertTriangle]  Building Permits — this month's update failed to download
    ...
    Impact: Building Permits data was not updated. Other topics are
            unaffected.
...
Severity icons (`lucide-react`): `AlertTriangle` (error, amber), `CheckCircle2` (success, green), `ShieldAlert` (recovered/fallback used, blue) — matches the `AlertTriangle`/`AlertCircle` and `RotateCcw` icons already used elsewhere in the app (e.g. `components/feedback/ReportProblemDialog.js`, `components/ui/*`). Phase names should use the same plain labels across topics (e.g. "Fetching source data," "Cleaning," "Publishing cleaned data") rather than each topic's internal step names.
