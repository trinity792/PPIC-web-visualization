...
description: >
  ...
  contain. Works alongside the markdown skill (which governs formatting) and whichever
...
- **By file.** Two unrelated fixes in one topic are two workstreams. One fix across nine topics is one.
...
Prefer prose to bullets for reasoning, tables for enumeration, and callouts only for the things that would genuinely be missed. Follow the project's `markdown-skill` for everything structural - heading depth, callout types, hyphens over em dashes, frontmatter, and the `Last Updated` discipline.
