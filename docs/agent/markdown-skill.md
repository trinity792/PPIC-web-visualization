...
description: >
  ...
  scale. Works alongside python-skill.md (which governs .py files) and
  frontend-skill.md (which governs .js/.jsx files).
Topic: Skill Guide
...
| `Content Type` | Yes | `agent instructions`, `analysis`, `archived guide`, `codebook`, `explainer`, `guide`, `implementation plan`, `landing page`, `notes`, `process documentation`, `project specification`, `reference guide`, `refractor plan`, `topic guide`, `topic specification`, `unit tests plan` | Tells the reader what kind of document this is. |
...
Used for each topic migration. Follows a consistent arc: legacy summary, unique challenges, target architecture, data contract, pipeline phases with function signatures, frontend deliverables, test plan, sequencing, resolved decisions, and open questions.

```markdown
# Topic Name: Refactoring Plan
...
Used for `AGENTS.md`, `CLAUDE.md`, and skill files. Structured as imperative rules with clear boundary markers.
...
Used for specifying exactly which tests to write. Organized by topic, with tables of test names and what each verifies. Includes fixture patterns and cross-cutting requirements.

```markdown
# Topic - Unit Testing Guide
...
- Use **kebab-case** for filenames: `building-permits-topic-guide.md`, not `BuildingPermits_Topic_Guide.md` or `building_permits_topic_guide.md`.
- Be descriptive: `age-sex-race-projections-topic-guide.md`, not `topic-guide-3.md`.
...
- Retired landing pages belong in `docs/archive/` with `Status: Archive`.
- Skill and instruction files use descriptive names: `python-skill.md`, `frontend-skill.md`, `AGENTS.md`.
...
pinned: false
description: "Refactoring plan for migrating the legacy X topic into the V3 architecture."
Date Published: June 30, 2026
Last Updated: 06/30/2026 - 10:17 AM
Status: Draft
---
...
A refactoring plan starts as `Draft` while it is being written against the legacy topic, moves to `Updating` once the migration is underway and the plan reflects what was actually built, and ends at `Finalized` when the topic ships.
