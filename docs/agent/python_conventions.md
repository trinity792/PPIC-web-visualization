...
def validate_dataframe(df, expected_cols):
    """Check that all expected columns are present. Raises ValueError if not."""
...
def clean_nulls(df):
    """Drop rows where critical fields are null; fill others with defaults."""
...
def process_state(state_fips, df):
    """Process a single state's data and return a dict ready for JSON export."""
...
def run_pipeline(states=None):
    """Run the full pipeline. If states is None, process all states."""
...
Every function gets a one-line docstring. Multi-line only when the function has non-obvious parameters or return values:
