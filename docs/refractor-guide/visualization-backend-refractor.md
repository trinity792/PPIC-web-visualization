...
That pipeline works for the datasets and questions it was originally designed around. The first topics emphasized population and housing data, where a single measure, geography, and set of filters were usually enough. Age, sex, and race projections place greater demands on the same model.
...
- `Group` and `Series` have different meanings in different parts of the system. In some topic-backed charts they can be stored without affecting the finished chart.
...
>   - **Response**: Use a searchable checklist of available years. For projections, default to the topic's declared reporting year. The current fixture should use 2025, which is the latest observed Census estimate year represented in the repository, rather than infer the default from the end of the 2070 projection range.
...
>   - **Response**: Clear the active time selection and show `Select time to show this chart`, consistent with the current topic and topic editor.
...
1. **Derive representative questions.** Build the initial examples from the accepted comparison and fixture contracts during implementation. If those examples expose a gap, return to the product decision instead of adding a topic-specific exception.
...
4. **Build the age, sex, and race case as the proving ground.** It is the most demanding current dataset and will expose assumptions that simpler topics do not.
...
6. **Convert the remaining chart families behind the same cutover boundary.** Reuse the same question and observation models rather than adding topic-specific branches. Do not expose a mixed old-and-new editor.
